...
int interaction::NN_Worker::init(const topology::Topology& topo
  ...
  if(sim.param().qmmm.nn.learning_type == simulation::nn_learning_type_all) {
    DEBUG(11, "device: " << device);
    ml_calculator = py_modules["schnetpack"].attr("interfaces").attr("gromos_wrapper").attr("calculator")("model_directory"_a=model_path.cast<std::string>(), "device"_a=device).attr("spk_calculator")();
  }  else {
  ...
  if (!sim.param().qmmm.nn.val_model_paths.empty()) {
    for (std::vector<std::string>::iterator it = sim.param().qmmm.nn.val_model_paths.begin(); it != sim.param().qmmm.nn.val_model_paths.end(); ++it) {
        ...
        if(sim.param().qmmm.nn.learning_type == simulation::nn_learning_type_all) {
          val_calculators.push_back(py_modules["schnetpack"].attr("interfaces").attr("gromos_wrapper").attr("calculator")("model_directory"_a=val_model_path.cast<std::string>(), "device"_a=device).attr("spk_calculator")());
        }
    }
  } 
...
int interaction::NN_Worker::run_QM(topology::Topology& topo
  ...
  // Write the energy
  DEBUG(13, "before get_potential_energies from NN, ");
  ...
  for (unsigned i = 0; it != to; ++it, ++i) {
    ...
    DEBUG(13, "force from NN, atom " << it->index << " : " << math::v2s(it->force));
  }
...
// Run validation, if asked for
  if (!sim.param().qmmm.nn.val_model_paths.empty()
    ...
    // else report the estimate of the standard deviation over all runs
    else {
      double sum = 0.0;
      double var = 0.0;
