...
int interaction::Colvar_Restraint_Interaction::init(topology::Topology &topo, 
		     ...
		     bool quiet) 
{
  if (!topo.coordnum_restraint().empty()) {
    for (size_t i=0; i< topo.coordnum_restraint().size(); i++) {
      interaction::Coordnum_Colvar *cv = new interaction::Coordnum_Colvar();
      (*cv).params = &topo.coordnum_restraint()[i];
