...
int interaction::Perturbed_Colvar_Restraint_Interaction::init(topology::Topology &topo, 
		     ...
		     bool quiet) 
{
  if (!topo.perturbed_coordnum_restraint().empty() and (sim.param().perturbation.perturbation)) {
    for (size_t i=0; i< topo.perturbed_coordnum_restraint().size(); i++) {
      interaction::Perturbed_Coordnum_Colvar *cv = new interaction::Perturbed_Coordnum_Colvar();
      (*cv).params = &topo.perturbed_coordnum_restraint()[i];
