			    ...
			    simulation::Simulation & sim) // ANITA
{
       ...
       ++it, ++k)
  {
    ...
    if (topo.inverse_mass(first + it->i) == 0 &&
        topo.inverse_mass(first + it->j) == 0)
      continue;
    ...
    const double lam_derivative = topo.individual_lambda_derivative(simulation::bond_lambda)
                                      [topo.atom_energy_group()[it->i]][topo.atom_energy_group()[it->i]];
    ...
    if(fabs(diff) >= constr_length2 * tolerance * 2.0){
      ...
      conf.old().perturbed_energy_derivatives.constraints_energy[topo.atom_energy_group()[it->i]] +=
 ...
 */
template <math::boundary_enum B, math::virial_enum V>
void algorithm::Perturbed_Shake ::perturbed_solute(topology::Topology const &topo,
                                                   configuration::Configuration &conf,
                                                   ...
                                                   int &error)
{
  ...
  while (!convergence)
  {
        ...
        sim.param().constraint.ntc > 1)
    {
      ...
      if (perturbed_shake_iteration<B, V>(topo, conf, m_solute_tolerance, pert_dist_convergence, first, skip_now, skip_next,
        ...
        sim.param().constraint.ntc > 1)
    {
      ...
      if (Shake::shake_iteration<B, V>(topo, conf, m_solute_tolerance, dist_convergence,
    ...
    // angle constraints
    bool ang_convergence = true, pert_ang_convergence = true;
    if (sim.param().angrest.angrest == simulation::angle_constr)
    {
      DEBUG(7, "SHAKE: perturbed angle constraints iteration");
      if (perturbed_ang_constr_iteration<B, V>(topo, conf, sim, pert_ang_convergence, skip_now, skip_next, periodicity))
      {
        io::messages.add("SHAKE error: perturbed angle constraints",
                         "Shake::solute",
                         io::message::error);
        std::cout << "SHAKE failure in solute perturbed angle constraints!" << std::endl;
        error = E_SHAKE_FAILURE_SOLUTE;
        break;
      }

      DEBUG(7, "SHAKE: angle constraints iteration");
      if (ang_constr_iteration<B, V>(topo, conf, sim, ang_convergence, skip_now, skip_next, topo.angle_restraints(), periodicity))
      {
    ...
    if (sim.param().dihrest.dihrest == simulation::dihedral_constr)
    {
      ...
      if (perturbed_dih_constr_iteration<B, V>(topo, conf, sim, pert_dih_convergence, skip_now, skip_next, periodicity))
      {
      ...
      if (dih_constr_iteration<B, V>(topo, conf, sim, dih_convergence, skip_now, skip_next, topo.dihedral_restraints(), periodicity))
      {
    ...
    convergence = pert_dist_convergence && dist_convergence && pert_dih_convergence && dih_convergence;
 ...
 */
int algorithm::Perturbed_Shake ::apply(topology::Topology &topo,
                                       configuration::Configuration &conf,
                                       simulation::Simulation &sim)
{
  ...
  if (!quiet)
  {
    ...
    if (sim.param().constraint.solvent.algorithm == simulation::constr_shake)
    {
      if (sim.mpi_enabled())
        ...
        os << "ON\n";
      os << "\t\ttolerance = "
         << std::scientific << sim.param().constraint.solvent.shake_tolerance << "\n";
    }
    else
      os << "OFF\n";
    os << "END\n";
