...
## 11. `rf_excluded` GPU support + extended_test/ubiquitin real-scale validation

`rf_excluded` (reaction-field correction for excluded pairs) had no GPU
implementation at all -- a stale `check_parameter.cc` "gpu" feature
matrix hard-locked several other already-implemented combinations too
(bond/angle/dihedral/improper, crf/lj, com_removal, pairlist/cutoff
selectors, multi-energy-groups, solute/solvent LINCS/SETTLE), all fixed
in one pass by mapping the reported errors to their exact `Feature`
keys and unlocking only what has a real, tested implementation.
`rf_excluded` itself then got a real kernel
(`rf_excluded_kernels.h`/`.cu`, solute self-term + exclusion-list pairs,
plus rigid-solvent chargegroup pairs), mirroring `RF_excluded_outerloop`.

Added `extended_test/`: a real ubiquitin-in-water system (~22.7k atoms)
as a GPU-vs-CPU regression test (`ubiquitin_cpu`/`ubiquitin_gpu.t.cc`,
driven via `Algorithm_Sequence` exactly like `program/md`, not a
narrower harness) -- deliberately much larger and structurally richer
(real multi-atom chargegroups, a real bonded/1,4 network) than any
existing GPU test topology. This immediately surfaced five previously-
undetected bugs, none reachable at aladip's scale: a thrust/cub crash
at large `n`, an unclamped `TileVecT::size()`, a silently-underestimated
candidate-tile capacity, an O(num_groups) dynamic-shared-memory ceiling
in the temperature-reduction kernel, and -- the deepest one -- 1,4-pair
("LJ exception") interactions never having been ported to GPU at all
(found via systematic elimination: pairlist verified exact, floating-
point precision ruled out by a full-double rebuild giving the identical
wrong answer, long-range caching ruled out by disabling it). Full
writeup, including a sixth bug found while validating the fifth
(`Nonbonded_Parameter::m_coulomb_scaling` had no default and was
dropped by the copy constructor) and a still-open, low-priority
intermittent flake in `rf_excluded_gpu`: `KNOWN_ISSUES.md`.

`ubiquitin_cpu`/`ubiquitin_gpu` are now permanent regression tests
(`ubiquitin_cpu` runs on every build, CPU-only included, like
`feature_checker_gpu`; `ubiquitin_gpu` is `USE_CUDA`-gated). The CPU
reference (`extended_test/reference/ubiquitin_cpu_reference.txt`) is
checked in, generated by the standalone `ubiquitin_generate_reference`
tool (not itself a ctest test) -- re-run and review its diff whenever
the underlying physics genuinely changes.

## 12. Open questions (revisit later, not blocking the plan above)
