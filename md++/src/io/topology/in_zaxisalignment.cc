...
void
io::In_Zaxisalignment::read(topology::Topology& topo,
              simulation::Simulation & sim,
              std::ostream & os){

  DEBUG(7, "reading in a z-axis angle restraints file");

  if (!quiet)
    os << "Z-AXIS ANGLE RESTRAINTS\n";
  read_ZALIGNMENTRESSPEC(topo, sim, os);

  if (!quiet) os << "END\n";

  if (sim.param().zalignmentres.zalignmentres) {
    if (block_read.count("ZALIGNMENTRESSPEC") == 0)
     /**&& block_read.count("PERTDISRESSPEC") == 0
     && block_read.count("MDISRESSPEC") == 0)*/
      io::messages.add("Z-axis angle restraints are on but no ZALIGNMENTRESSPEC found",
               "In_Zaxisalignment",
               io::message::error);
  }
 ...
 */
void io::In_Zaxisalignment::read_ZALIGNMENTRESSPEC(topology::Topology &topo,
                      simulation::Simulation &sim,
                      std::ostream & os)  { // DISTANCERES
  DEBUG(10, "ZALIGNMENTRESSPEC block");
  ...
  exampleblock << "ZALIGNMENTRESSPEC\n";
  ...
  std::string blockname = "ZALIGNMENTRESSPEC";
  ...
  if (block.read_buffer(m_block[blockname], false) == 0) {
    ...
    DEBUG(10, "reading in ZALIGNMENTRES data");

    if (!quiet) {
        switch (sim.param().zalignmentres.zalignmentres) {
          case 0:
            os << "\tZ-axis angle restraints OFF\n";
            // how did you get here?
            break;
          case 1:
            os << "\tZ-axis angle restraints ON\n";
            break;
          case -1:
            os << "\tZ-axis angle restraints ON\n"
          ...
          case -2:
            ...
            break;
          default:
            os << "\tZ-axis angle restraints ERROR\n";
        }
    }
    ...
    for(unsigned int line_number=0; line_number < num; line_number++){
      ...
      for(unsigned int i = 0; i < io::In_Zaxisalignment::MAX_ATOMS; i++) {
        unsigned int atom;
        ...
        if (atom > topo.num_atoms()) {
          ...
          io::messages.add(msg.str(), "In_Zaxisalignment", io::message::error);
        }
    ...
    // g++ 3.2 fix
      if(!block.error()){
        ...
        topo.zalignment_restraints().push_back
          (topology::zalignment_restraint_struct(v1,v2,a0,w0,rah));
