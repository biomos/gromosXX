...
(
  ...
  if (sim.param().tfrdc.mode == simulation::tfrdc_restr_av ||
    ...
    buffer = m_block["TFRDCRESEXPAVE"];
    if (buffer.size()) {
      block_read.insert("TFRDCRESEXPAVE");
      if (!quiet)
      ...
      if (sim.param().tfrdc.read)
        io::messages.add("No TFRDCRESEXPAVE block in configuration.",
              "In_Configuration", io::message::error);
    }
  }
...
bool io::In_Configuration::_read_order_parameter_restraint_averages(
  ...
  DEBUG(8, "read order parameter restraint averages");
