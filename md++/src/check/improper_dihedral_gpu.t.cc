...
/**
 * @file improper_dihedral_gpu.t.cc
 * End-to-end correctness test for CUDA_Improper_Dihedral_Interaction
 * (PLAN.md §10 step 14, bonded forces) -- runs the real CPU
 * Improper_Dihedral_Interaction and the real
 * CUDA_Improper_Dihedral_Interaction on aladip's unperturbed topology/
 * configuration and compares the resulting per-atom forces,
 * per-energy-group improper energies, and atomic virial tensor. Only
...
namespace {
  ...
  int run_case(const std::string & stopo, const std::string & sconf,
    ...
    for (auto & e : cpu_s.conf.current().energies.improper_energy) e = 0.0;
    for (auto & e : gpu_s.conf.current().energies.improper_energy) e = 0.0;

    interaction::Improper_Dihedral_Interaction cpu_improper;
    interaction::CUDA_Improper_Dihedral_Interaction gpu_improper;

    if (cpu_improper.init(cpu_s.topo, cpu_s.conf, cpu_s.sim, std::cout, quiet) != 0 ||
        gpu_improper.init(gpu_s.topo, gpu_s.conf, gpu_s.sim, std::cout, quiet) != 0) {
      flush_messages("improper_dihedral init");
      std::cerr << label << ": init() failed" << std::endl;
      return 1;
    }
    flush_messages("improper_dihedral init");

    if (cpu_improper.calculate_interactions(cpu_s.topo, cpu_s.conf, cpu_s.sim) != 0 ||
        gpu_improper.calculate_interactions(gpu_s.topo, gpu_s.conf, gpu_s.sim) != 0) {
    ...
    const unsigned num_energy_groups =
        static_cast<unsigned>(cpu_s.conf.current().energies.improper_energy.size());
    for (unsigned g = 0; g < num_energy_groups; ++g) {
      const double cpu_e = cpu_s.conf.current().energies.improper_energy[g];
      const double gpu_e = gpu_s.conf.current().energies.improper_energy[g];
      if (std::abs(cpu_e - gpu_e) > tol * std::max(1.0, std::abs(cpu_e))) {
        std::cerr << label << ": improper_energy[" << g << "] mismatch: cpu=" << cpu_e
                  << " gpu=" << gpu_e << std::endl;
        ++errors;
      }
    }
...
int main(int argc, char* argv[]) {
  ...
  return run_case(stopo, sconf, sinput, "improper_dihedral_gpu", quiet);
}
