...
/**
 * @file dihedral_gpu.t.cc
 * End-to-end correctness test for CUDA_Dihedral_Interaction (PLAN.md
 * §10 step 14, bonded forces) -- runs the real CPU
 * Dihedral_new_Interaction and the real CUDA_Dihedral_Interaction on
 * aladip's unperturbed topology/configuration and compares the
 * resulting per-atom forces, per-energy-group dihedral energies, and
 * atomic virial tensor. Only USE_CUDA builds run this.
 */
...
namespace {
  ...
  int run_case(const std::string & stopo, const std::string & sconf,
    ...
    // aladip_unperturbed.in turns dihedral-angle-minimum monitoring on,
    // a CPU-only diagnostic (conf.special().dihangle_trans) that
    // CUDA_Dihedral_Interaction hard-errors on rather than silently
    // ignoring (see dihedral_kernels.h's doc comment) -- it doesn't
    // affect the force/energy formula this test compares, so disable it
    // for both sides rather than switching to a different input file.
    cpu_s.sim.param().print.monitor_dihedrals = false;
    gpu_s.sim.param().print.monitor_dihedrals = false;

    if (cpu_s.sim.param().force.dihedral != 1) {
      std::cerr << label << ": expected param.force.dihedral == 1 for this "
                << "topology/input (got " << cpu_s.sim.param().force.dihedral
                << ") -- CUDA_Dihedral_Interaction only matches "
                << "Dihedral_new_Interaction" << std::endl;
      return 1;
    }
    ...
    for (auto & e : cpu_s.conf.current().energies.dihedral_energy) e = 0.0;
    for (auto & e : gpu_s.conf.current().energies.dihedral_energy) e = 0.0;

    interaction::Dihedral_new_Interaction cpu_dihedral;
    interaction::CUDA_Dihedral_Interaction gpu_dihedral;

    if (cpu_dihedral.init(cpu_s.topo, cpu_s.conf, cpu_s.sim, std::cout, quiet) != 0 ||
        gpu_dihedral.init(gpu_s.topo, gpu_s.conf, gpu_s.sim, std::cout, quiet) != 0) {
      flush_messages("dihedral init");
      std::cerr << label << ": init() failed" << std::endl;
      return 1;
    }
    flush_messages("dihedral init");

    if (cpu_dihedral.calculate_interactions(cpu_s.topo, cpu_s.conf, cpu_s.sim) != 0 ||
        gpu_dihedral.calculate_interactions(gpu_s.topo, gpu_s.conf, gpu_s.sim) != 0) {
      flush_messages("dihedral calculate_interactions");
      std::cerr << label << ": calculate_interactions() failed" << std::endl;
      return 1;
    }
    flush_messages("dihedral calculate_interactions");

    // CUDA_Dihedral_Interaction writes force directly into the GPU
...
int main(int argc, char* argv[]) {
  ...
  return run_case(stopo, sconf, sinput, "dihedral_gpu", quiet);
}
