...
class SchNet_V2_Calculator:
    ...
    def _b2_eval_model(
        ...
        phi1 = self._coulomb_phi(r_qeq_A, r_or_A, q_or)
    ...
    def _coulomb_phi(self, r_qeq_A, r_or_A, q_or_e,make_zero=False):
        """
        Compute phi_i = k_e * sum_j q_j / r_ij (plain Coulomb with cutoff)
        Units:
            r in Ang
            q in e
            returns phi in kJ/mol per e per Ang
        """

        if r_or_A.shape[0] == 0:
            return torch.zeros(r_qeq_A.shape[0], device=r_qeq_A.device)
        ...
        phi = K_E_EV_ANG_E2 * torch.sum(inv_r * q_or_e[None, :], dim=1)
        
        if make_zero:
            phi = 0* K_E_EV_ANG_E2 * torch.sum(inv_r * q_or_e[None, :], dim=1)


        # safe scalar prints
        #max_phi = phi.max().detach().cpu().item()
        #min_dist = dist.min().detach().cpu().item()

        #print(f"max phi eV/e: {max_phi:.6f}")
        #print(f"min dist nm:  {min_dist:.6f}")
