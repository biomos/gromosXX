...
class SchNet_V2_Calculator:
    ...
    def __init__(self, model_path: str, val_model_paths: list,
        ...
        self.default_sigma_or = 1.60

    def _sigma_or_from_Z(self, z_or: torch.Tensor) -> torch.Tensor:
        vals = [self.sigma_or_table.get(int(z), self.default_sigma_or)
                for z in z_or.detach().cpu().tolist()]
        return torch.tensor(vals, device=z_or.device, dtype=torch.float32)
    ...
    def _energy_forces(self, calculator, system, r_mlp_A):
        inputs = calculator.converter(system)
        inputs[spk.properties.R] = r_mlp_A  # <-- use the passed tensor
        ...
        F_mlp = (-dE_dR).detach().cpu().numpy()
        return float(E_mlp.detach().cpu().item()), F_mlp

    def predict_energy_and_forces(self, system: ase.Atoms,r_mlp_A: torch.Tensor,):
        """
        Predict energy and forces for the given atomic system.
        ...
        # Perform predictions.
        return self._energy_forces(system=system, calculator=self.pred_calculator,r_mlp_A=r_mlp_A)
    ...
    def validate_prediction(
        ...
        sigma_or: torch.Tensor | None = None
    ) -> list[float]:
        """
        ...
        if not dynamic_charges:
            ...
            for val_calculator in self.val_calculators:
                e_kj, _ = self._energy_forces(system=system, calculator=val_calculator)
    ...
    def validate_prediction_maxForceDeviation(
        ...
        if not dynamic_charges:
            ...
            for val_calculator in self.val_calculators:
                _, f_kj_A = self._energy_forces(system=system, calculator=val_calculator)
    ...
    def _pair_energy_screened(
        self,
        r_A: torch.Tensor,          # (N,3)
        q_e: torch.Tensor,          # (N,)
        sigma_A: torch.Tensor | None = None,   # (N,) or None
        cutoff_A: float | None = None,
    ):
        """
        Pair energy:
            1/2 * sum_{i != j} q_i q_j * k_e * erf(r_ij / sigma_ij) / r_ij

        If sigma_A is None -> bare Coulomb kernel.
        Units:
            r in Ang
            q in e
        ...
        n = r_A.shape[0]
        if n == 0:
            return torch.zeros((), device=r_A.device, dtype=r_A.dtype)

        diff = r_A[:, None, :] - r_A[None, :, :]
        dist = torch.norm(diff, dim=-1).clamp_min(1e-8)   # (N,N)

        eye = torch.eye(n, device=r_A.device, dtype=torch.bool)

        if sigma_A is None:
            kernel = 1.0 / dist
        else:
            sigma_A = sigma_A.to(device=r_A.device, dtype=r_A.dtype)
            sigma_ij = torch.sqrt(
                sigma_A[:, None] ** 2 + sigma_A[None, :] ** 2
            ).clamp_min(1e-8)
            kernel = torch.erf(dist / sigma_ij) / dist
        ...
        qq = q_e[:, None] * q_e[None, :]
        E_eV = 0.5 * K_E_EV_ANG_E2 * torch.sum(qq * kernel)
        return E_eV
    ...
    def _b2_eval_model(
        ...
        def _forward_with_phi(phi_tensor):
            inputs = calculator.converter(system)
            inputs[spk.properties.R] = r_qeq_A
            inputs["phi"] = phi_tensor
        ...
        phi0 = torch.zeros(r_qeq_A.shape[0], device=r_qeq_A.device, dtype=r_qeq_A.dtype)

        # pass 1: no external field, just to get internal QEq descriptors such as sigma_qeq
        out0 = _forward_with_phi(phi0)

        # try to use learned sigma from QEq head if available
        sigma_qeq = out0.get("sigma_qeq", None)


        # screened external potential
        phi1 = self._coulomb_phi(
            r_qeq_A=r_qeq_A,
            r_or_A=r_or_A,
            q_or_e=q_or,
            sigma_qeq_A=sigma_qeq,
            sigma_or_A=sigma_or,        # start simple; later you can make this element-dependent
            cutoff_A=cutoff_Ang,   # uses your existing cutoff
        )

        # pass 2: actual embedded calculation
        out1 = _forward_with_phi(phi1)

        q0 = out0["charges"]
        q1 = out1["charges"]
        
        print("gamma_qeq =", float(out0["gamma_qeq"].detach().cpu()))
        print("sigma_qeq min/max/mean =",
            float(out0["sigma_qeq"].min().detach().cpu()),
            float(out0["sigma_qeq"].max().detach().cpu()),
            float(out0["sigma_qeq"].mean().detach().cpu()))
        print("max |q| =", float(q1.abs().max().detach().cpu()))

        print("max |q1-q0| =", float((q1 - q0).abs().max().detach().cpu()))
        print("mean |q1-q0| =", float((q1 - q0).abs().mean().detach().cpu()))# e
        #print("Charges without elecpot: ",q0.detach().cpu().numpy())
        #print("Charges with elecpot: ",q1.detach().cpu().numpy())
        ...
        def _topk_abs(name, t, k=5):
            t = t.detach().flatten()
            vals, idx = torch.topk(torch.abs(t), k=min(k, t.numel()))
            return [(int(i.cpu()), float(v.cpu()), float(t[int(i)].cpu())) for v, i in zip(vals, idx)]
        print("N_or =", int(r_or_A.shape[0]))
        print(_summ("chi eV", chi))
        print(_summ("Jii eV", Jii))
        print(_summ("phi eV", phi1))
        print(_summ("q", q1))
        diff = r_qeq_A[:, None, :] - r_or_A[None, :, :]
        dist = torch.norm(diff, dim=-1)  # Å
        dmin, flat = dist.view(-1).min(dim=0)
        i = int(flat // dist.shape[1])
        j = int(flat %  dist.shape[1])
        print("min d(IR-OR) Å =", float(dmin.detach().cpu()), "i=", i, "j=", j, "q_or=", float(q_or[j].detach().cpu()))
        #phi_ij = 14.399645 * q_or[None, :] / dist.clamp_min(1e-6)  # eV/e, (Nq,Nor)
        """if "sigma_qeq" in out0:
            sigma_q = out0["sigma_qeq"].detach()
            sigma_or = torch.full(
                (r_or_A.shape[0],),
                1.0,
            ...
            sigma_ij = torch.sqrt(sigma_q[:, None] ** 2 + sigma_or[None, :] ** 2).clamp_min(1e-8)
            phi_ij = 14.399645 * q_or[None, :] * torch.erf(dist / sigma_ij) / dist.clamp_min(1e-6)
        else:
            phi_ij = 14.399645* q_or[None, :] / dist.clamp_min(1e-6)"""
        #i_star = int(torch.argmin(phi1).detach().cpu())
        #vals, idx = torch.topk(-phi_ij[i_star], k=min(5, phi_ij.shape[1]))  # most negative contributions
        #print("i*=", i_star, "phi(i*)=", float(phi1[i_star].detach().cpu()))
        #print("top negative contribs:", [(int(jj), float(phi_ij[i_star, jj].detach().cpu())) for jj in idx])

        #print("top |phi|:", _topk_abs("phi", phi, k=5))
        #print("top |q|:",   _topk_abs("q", q,   k=5))
        #print("top |chi|:", _topk_abs("chi", chi, k=5))
        #print("min Jii indices:", torch.topk((-Jii).detach().flatten(), k=5).indices.detach().cpu().tolist())
        
        E_mlp = out1[calculator.energy_key].sum()  # kJ/mol (per your SpkCalculator config)
        E_qphi_eV = torch.sum(q1 * phi1)             # eV
        E_qphi_kJ = E_qphi_eV * EV_TO_KJMOL        # kJ/mol
        E_total = E_mlp + E_qphi_kJ 
        print(out1.keys())
        print("E_MLP :", E_mlp)
        
        E0 = out0[calculator.energy_key].sum()
        E1 = out1[calculator.energy_key].sum()
        print("ΔE_model (kJ/mol) =", float((E1-E0).detach().cpu()))
        ...
        #print("Forces total IR",F_total_qeq)
        #print("Forces OR",F_total_or)
    ...
    def calculate_next_step(
        ...
        cutoff_nm: float | None = None,
        or_atomic_numbers: list | None = None    ) -> None:
        """
        Unified next-step:
        ...
        if not dynamic_charges:
            ...
            self.energy, self.forces = self.predict_energy_and_forces(system=system,r_mlp_A=r_mlp_A)
        ...
        if len(or_positions_nm) > 0:
            ...
            sigma_or = self._sigma_or_from_Z(z_or)
        else:
        ...
        #print("OR atoms in pairlist :", len(or_positions_nm))
        # what MD uses
    ...
    def _coulomb_phi(
        ...
        r_or_A: torch.Tensor,
        q_or_e: torch.Tensor,
        sigma_qeq_A: torch.Tensor | None = None,
        sigma_or_A: float | torch.Tensor = 1.0,
        cutoff_A: float | None = None,
        ...
        if sigma_qeq_A is None:
            ...
            # sigma_qeq_A: (Nq,)
            sigma_qeq_A = sigma_qeq_A.to(device=r_qeq_A.device, dtype=r_qeq_A.dtype)

            # sigma_or_A: scalar or (Nor,)
            if torch.is_tensor(sigma_or_A):
                sigma_or = sigma_or_A.to(device=r_qeq_A.device, dtype=r_qeq_A.dtype)
            else:
                sigma_or = torch.full(
                    (r_or_A.shape[0],),
                    float(sigma_or_A),
            ...
            sigma_ij = torch.sqrt(
                sigma_qeq_A[:, None] ** 2 + sigma_or[None, :] ** 2
            ).clamp_min(1e-8)
        ...
        phi = K_E_EV_ANG_E2 * torch.sum(kernel * q_or_e[None, :], dim=1)
    ...
    def _coulomb_phi_gromos_rf(
        ...
        eps_rf: float = 61.0,
        screened: bool = True,
        sigma_A: float = 0.02,   # ~0.2 Å; tune (0.02–0.05 nm typical)
        kappa_inv_A: float | None = None,  # optional Debye screening (1/A)
    ):
        """
        GROMOS-style reaction-field potential at QEq sites due to OR charges.

        If screened=True: uses Gaussian-smeared Coulomb:
            coul(r) = erf(r/(sqrt(2)*sigma)) / r
        Optionally multiply by Debye factor exp(-kappa*r).
        """

        if r_or_A.shape[0] == 0:
            return torch.zeros(r_qeq_A.shape[0], device=r_qeq_A.device, dtype=r_qeq_A.dtype)

        diff = r_qeq_A[:, None, :] - r_or_A[None, :, :]
        r2 = torch.sum(diff * diff, dim=-1)  # (Nq, Nor)
        r = torch.sqrt(r2).clamp_min(1e-8)   # numeric guard only

        rc = float(cutoff_A)

        # reaction-field constant
        crf = (eps_rf - 1.0) / (2.0 * eps_rf + 1.0)
        m_2cut3i = crf / (2.0 * rc**3)
        m_cut    = (1.0 - 0.5 * crf) / rc

        # Coulomb part
        if screened:
            sig = float(sigma_A)
            x = r / (math.sqrt(2.0) * sig)
            coul = torch.erf(x) / r
        else:
            coul = 1.0 / r

        if kappa_inv_A is not None:
            coul = coul * torch.exp(-float(kappa_inv_A) * r)

        # RF-shifted kernel (same structure as GROMOS)
        kernel = coul - (m_2cut3i * r2) - m_cut
