 ...
 *
 * @anchor eds_update_1
 * @section eds_update_1 calculates the parameters s and EiR iteratively for multiple EDS states
 * @author @ref sr
 * @date 27. 01. 10
 *
  ...
  try {
    ...
    switch (form) {
      ...
      case 3:
        ...
        break;
      default:
        throw gromos::Exception("eds_update_1", "functional form must be 1,2 or 3");
        break;
    }
    ...
    vector<double> eir_old(numstat);
    {
      ...
      for (Arguments::const_iterator iter = args.lower_bound("EiR"),
        ...
        if (!(is >> eir_old[i])) {
          throw gromos::Exception("eds_update_1", "EiR not numeric or too many values");
        }
      }
      if (i < numstat)
        throw gromos::Exception("eds_update_1", "not enough EiR values (must be N)");
    }
    ...
    args.check("s", 1);
    {
      if (form > 1) {
        ...
        for (Arguments::const_iterator iter = args.lower_bound("s"),
          ...
          std::istringstream is(iter->second.c_str());
          if (!(is >> pairs[i].s_old)) {
            throw gromos::Exception("eds_update_1", "s not numeric or too many values");
          }
        }
        if (form == 2 && i < numpairs1)
          throw gromos::Exception("eds_update_1", "not enough s values (must be N(N-1)/2)");
        if (form == 3 && i < numpairs3)
          throw gromos::Exception("eds_update_1", "not enough s values (must be (N-1))");
      } else {
        std::istringstream is(args["s"]);
        if (!(is >> smallest_s_old)) {
          throw gromos::Exception("eds_update_1", "functional form not numeric");
        }
      }
    }
    ...
    if (form == 3) {
      ...
      args.check("update_tree", 1);
      {
        ...
        if (!(is >> tree)) {
          throw gromos::Exception("eds_update_1", "updating tree switch not numeric");
        }
        if (tree != 0 && tree != 1)
          throw gromos::Exception("eds_update_1", "update_tree must be 0 or 1");
      }
      // read tree
      if (tree == 0 || tree == 1) {
        args.check("tree", 1);
        ...
        if (!file.good()) {
          throw gromos::Exception("eds_update_1", "Could not open file '" + args["tree"] + "'");
        }
        if (!file.is_open()) {
          throw gromos::Exception("eds_update_1", "could not open file '" + args["tree"] + "'");
        }
        ...
        do {
          ...
          if (!file.eof() && line[0] != '#') {
            ...
            if (!linestream.good() && !linestream.eof()) {
              ...
              throw gromos::Exception("eds_update_1", os.str());
            }
            pairs[i].i = A - 1;
            pairs[i].j = B - 1;
        ...
        file.close();
        if (i < numpairs3)
          throw gromos::Exception("eds_update_1", "not enough pairs (must be (N-1))");
      }
    } else {
      // assign pairs
      unsigned int p = 0;
    ...
    if (i < numstat)
      throw gromos::Exception("eds_update_1", "not enough energy time series vy");

    //cout << "everything read in" << endl;

    // check whether time series have the same length
    if (vy[0].n() != vr.n())
      throw gromos::Exception("eds_update_1", "Time series files differ in length!\n");
...
gmath::Stat<double> read_data(string name, Arguments::const_iterator & iter) {
  ...
  if (!x)
    throw gromos::Exception("eds_update_1", "Could not open time series file for " + name + ".\n");
  ...
  while (true) {
    ...
    if ((!(is >> time >> q)) && !x.eof())
      throw gromos::Exception("eds_update_1", "Error when reading from " + name + " time series file.\n");
    data.addval(q);
