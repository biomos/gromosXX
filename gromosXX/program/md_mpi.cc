...
int main(int argc, char *argv[]) {
    ...
    MPI_Comm_split(MPI_COMM_WORLD, sim.mpi_control.simulationMPIColor, rank, &simulationCOMM);
    ...
    sim.mpi_control.simulationNumberOfThreads = simulation_size;
    sim.mpi_control.simulationThisThreadID = simulation_rank;
    sim.mpi_control.simulationOwnedThreads = simulationOwnedThreads;
    sim.mpi_control.simulationCOMM = simulationCOMM;
    ...
    if (sim.mpi_control.simulationThisThreadID == sim.mpi_control.simulationMasterThreadID) {
        std::cout << "MPI INFORMATION \n" <<
                ...
                "\tSimulation Master Thread ID " << sim.mpi_control.simulationMasterThreadID << "\n" <<
                "\tSimulation Total Num Threads " << sim.mpi_control.simulationNumberOfThreads << "\n" <<
    ...
    std::cout.flush();
    MPI_Barrier(simulationCOMM);
    std::cout << "Simulation This Thread ID " << sim.mpi_control.simulationThisThreadID << "\n";
    std::cout.flush();
    MPI_Barrier(simulationCOMM);
    std::cout.flush();
    if (sim.mpi_control.simulationThisThreadID == sim.mpi_control.simulationMasterThreadID) {
    ...
    if (sim.mpi_control.simulationThisThreadID == sim.mpi_control.simulationMasterThreadID) {
...

        {
            ...
            if (iom >= io::message::error) {
                ...
                FFTW3(mpi_cleanup());
                MPI_Comm_free(&sim.mpi_control.simulationCOMM); //Clean up
                MPI::Finalize();
                return 1;
        ...
        MPI_DEBUG(1, "MASTER " << rank << " INIT DONE");
        ...
        if (error) {
             ...
             */
            MPI_DEBUG(1, "slave " << rank << " INIT DONE");
