...
void _calculate_forces_vectors_MF(topology::Topology & topo,
  ...
  const int number_hvectors = conf.special().rdc.MFpoint.size();
  ...
  for(; it!=to; ++it, ++k) {
    ...
    conf.special().rdc.curr[k] = 0.0;
    // Loop over all magnetic field vectors
    for(int h=0; h<number_hvectors; ++h) {
      rh = conf.special().rdc.MFpoint.at(h); // [nm]
      conf.special().rdc.curr[k] +=  pow(math::dot(rij, rh) / dij, 2) - 1.0/3.0; // [1]
    } // Loop over MF
    conf.special().rdc.curr[k] *= RDC_max(it)/dij3 * 1.0/number_hvectors * 1.5; // [1/ps]
    ...
    if(sim.param().rdc.mode == simulation::rdc_restr_av ||
      ...
      double local_average = exp * conf.special().rdc.av[k] + (1.0 - exp) * conf.special().rdc.curr[k]; // [1/ps]
      ...
      DEBUG(10, "interaction " << k << ":  R-inst, R-av: " << conf.special().rdc.curr[k] << ", " << local_average )
    }
    ...
    for(int h=0; h<number_hvectors; ++h) {
      ...
      if(sim.param().rdc.mode == simulation::rdc_restr_inst ||
        ...
        const double dV_dD = force_coefficient * (conf.special().rdc.curr[k] - it->R0); // [kJ/(nm^2*mol)]
        ...
        DEBUG(15, "dV_dD; dD_dh: " << dV_dD << ", " << scientific << "(" << dD_dh[0] << ", " << dD_dh[1] << ", " << dD_dh[2] << ")")
              ...
              sim.param().rdc.mode == simulation::rdc_restr_av_weighted){
        const double dVav_dDav = force_coefficient * (local_average - it->R0); // [kJ/(nm^2*mol)]
        ...
        DEBUG(15, "dVav_dDav; dDav_dD; dD_dh: " << dVav_dDav << ", " << dDav_dD << ", " << scientific << "(" << dD_dh[0] << ", " << dD_dh[1] << ", " << dD_dh[2] << ")")
              ...
              sim.param().rdc.mode == simulation::rdc_restr_biq_weighted){
        const double dVbq_dD = force_coefficient * (conf.special().rdc.curr[k] - it->R0) * pow(local_average - it->R0, 2); // [kJ/(nm^2*mol)]
        const double dVbq_dDav = force_coefficient * pow(conf.special().rdc.curr[k] - it->R0, 2) * (local_average - it->R0); // [kJ/(nm^2*mol)]
	    // the following is wrong, but [1-e^(-dt/tau)] is constant and very small and
	    // would therefore require a different K.  Omitting this factor allows us
	    // to use the same K as before
        const double dDav_dD = 1.0;
      ...
      DEBUG(10, "mf vector (" << h << "): " << math::v2s(conf.special().rdc.MFpoint.at(h)) )
      DEBUG(10, "forces on the mf vectors(" << h << "): " << math::v2s(force_vectors_hfield[h]) )
      DEBUG(10, "velocity of vector component(" << h << "): " << math::v2s(conf.special().rdc.MFpointVel[h]))
    } // Loop over MF (Vphys)
  } // Loop over RDC

  for (int i=0; i<number_hvectors; ++i) {
    DEBUG(10, "position of h-vectors(" << i << "): " << math::v2s(conf.special().rdc.MFpoint.at(i)))
...
void _calculate_forces_atoms_MF(topology::Topology & topo,
  ...
  for(; it!=to; ++it, ++k) {
    ...
    if(sim.param().rdc.mode == simulation::rdc_restr_av ||
       ...
       sim.param().rdc.mode == simulation::rdc_restr_biq_weighted){
      const double exp = pow(M_E, -sim.time_step_size()/sim.param().rdc.tau); // [1]
      conf.special().rdc.av[k] *= exp; // [1/ps]
      conf.special().rdc.av[k] += (1.0 - exp) * conf.special().rdc.curr[k]; // [1/ps]
      DEBUG(15, "Delta t, tau, e^(- Delta t/tau): " << scientific << sim.time_step_size() << ", " <<  sim.param().rdc.tau << ", " << pow(M_E, -sim.time_step_size()/sim.param().rdc.tau) )
      DEBUG(10, "interaction " << k << ":  R-inst, R-av: " << conf.special().rdc.curr[k] << ", " << conf.special().rdc.av[k] )
    }
    ...
    if(sim.param().rdc.mode == simulation::rdc_restr_inst ||
      ...
      conf.current().energies.rdc_energy[topo.atom_energy_group()[it->i]] += 0.25 * force_coefficient * pow(conf.special().rdc.curr[k] - it->R0, 2); // [kJ/mol]
      conf.current().energies.rdc_energy[topo.atom_energy_group()[it->j]] += 0.25 * force_coefficient * pow(conf.special().rdc.curr[k] - it->R0, 2); // [kJ/mol]
      DEBUG(10, "interaction_energy (inst): " << scientific << 0.5 * force_coefficient * pow(conf.special().rdc.curr[k] - it->R0, 2))
    }
    else if(sim.param().rdc.mode == simulation::rdc_restr_av ||
            sim.param().rdc.mode == simulation::rdc_restr_av_weighted){
      // rdc_energy = .5 * K * (<RDC> - RDC_0)^2
      conf.current().energies.rdc_energy[topo.atom_energy_group()[it->i]] += 0.25 * force_coefficient * pow(conf.special().rdc.av[k] - it->R0, 2); // [kJ/mol]
      conf.current().energies.rdc_energy[topo.atom_energy_group()[it->j]] += 0.25 * force_coefficient * pow(conf.special().rdc.av[k] - it->R0, 2); // [kJ/mol]
      DEBUG(10, "interaction_energy (av): " << scientific << 0.5 * force_coefficient * pow(conf.special().rdc.av[k] - it->R0, 2))
    }
    else if(sim.param().rdc.mode == simulation::rdc_restr_biq ||
            sim.param().rdc.mode == simulation::rdc_restr_biq_weighted){
      // rdc_energy = .5 * K * (RDC - RDC_0)^2 * (<RDC> - RDC_0)^2
      conf.current().energies.rdc_energy[topo.atom_energy_group()[it->i]] += 0.25 * force_coefficient * pow(conf.special().rdc.curr[k] - it->R0, 2) * pow(conf.special().rdc.av[k] - it->R0, 2); // [kJ/mol]
      conf.current().energies.rdc_energy[topo.atom_energy_group()[it->j]] += 0.25 * force_coefficient * pow(conf.special().rdc.curr[k] - it->R0, 2) * pow(conf.special().rdc.av[k] - it->R0, 2); // [kJ/mol]
      DEBUG(10, "interaction_energy (biq): " << scientific << 0.5 * force_coefficient * pow(conf.special().rdc.curr[k] - it->R0, 2) * pow(conf.special().rdc.av[k] - it->R0, 2))
    }

    // forces on atoms
    math::Vec force_vector_atom(0.0, 0.0, 0.0);
    for (int h=0; h<number_hvectors; ++h) {
      rh = conf.special().rdc.MFpoint.at(h);
    ...
    if(sim.param().rdc.mode == simulation::rdc_restr_inst ||
      ...
      DEBUG(15, "dV_dD; dD_dr: " << dV_dD << ", (" << scientific << dD_dr[0] << ", " << dD_dr[1] << ", " << dD_dr[2] << ")")
            ...
            sim.param().rdc.mode == simulation::rdc_restr_av_weighted){
      const double dVav_dDav = force_coefficient * (conf.special().rdc.av[k] - it->R0); // [kJ/(nm^2*mol)]
	  // the following is wrong, but [1-e^(-dt/tau)] is constant and very small and
	  // would therefore require a different K.  Omitting this factor allows us
	  // to use the same K as before
      const double dDav_dD = 1.0;
      ...
      DEBUG(15, "dVav_dDav; dDav_dD; dD_dr: " << dVav_dDav << ", " << dDav_dD << ", (" << scientific << dD_dr[0] << ", " << dD_dr[1] << ", " << dD_dr[2] << ")")
            ...
            sim.param().rdc.mode == simulation::rdc_restr_biq_weighted){
      const double dVbq_dD = force_coefficient * (conf.special().rdc.curr[k] - it->R0) * pow(conf.special().rdc.av[k] - it->R0, 2); // [kJ/(nm^2*mol)]
      const double dVbq_dDav = force_coefficient * pow(conf.special().rdc.curr[k] - it->R0, 2) * (conf.special().rdc.av[k] - it->R0); // [kJ/(nm^2*mol)]
	  // the following is wrong, but [1-e^(-dt/tau)] is constant and very small and
	  // would therefore require a different K.  Omitting this factor allows us
	  // to use the same K as before
      const double dDav_dD = 1.0;
...
//    cout << "phi: " << phi << endl;
  ...
  for(; it!=to; ++it, ++k) {
    ...
    // Loop over all magnetic field vectors
    for(int h=0; h<number_hvectors; ++h) {
      conf.special().rdc.curr[k] +=  pow(math::dot(rij, h_vectors[h]) / dij, 2) - 1.0/3.0; // [1]
    } // Loop over MF
    conf.special().rdc.curr[k] *= RDC_max(it)/dij3 * 1.0/number_hvectors * 1.5; // [1/ps]
    ...
    potential += 0.5 * force_coefficient * pow(conf.special().rdc.curr[k] - it->R0, 2); // [kJ/mol]
...
void mf_gradient(const gsl_vector* MF, void* param, gsl_vector* gradient) {
  ...
  for(; it!=to; ++it, ++k) {
    ...
    // f = - K * (RDC - RDC_0) [ * inner derivative]
    for(int h=0; h<number_hvectors; ++h) {
      force_vectors_hfield[h] -= 3.0/dij5 * force_coefficient * (conf.special().rdc.curr[k] - it->R0) * RDC_max(it) * 1.0/number_hvectors * math::dot(rij, h_vectors[h]) * rij; // [kJ/(nm*mol)]
    } // Loop over MF (Vphys)
...
int _calculate_interactions_mfield(topology::Topology & topo,
  ...
  switch(sim.param().rdc.method) {
    case simulation::rdc_em: {
      ...
      const int number_magnetic_field_vectors = conf.special().rdc.MFpoint.size();
      ...
      for(int h=0; h<number_magnetic_field_vectors; ++h) {
        const double x = conf.special().rdc.MFpoint[h][0];
        const double y = conf.special().rdc.MFpoint[h][1];
        const double z = conf.special().rdc.MFpoint[h][2];
...
// cout << endl;
      ...
      for(int h=0; h<number_magnetic_field_vectors; ++h) {
        ...
        conf.special().rdc.MFpoint.at(h)[0] = r * sin(theta) * cos(phi); // [nm]
        conf.special().rdc.MFpoint.at(h)[1] = r * sin(theta) * sin(phi); // [nm]
        conf.special().rdc.MFpoint.at(h)[2] = r * cos(theta); // [nm]
      ...
      vector<topology::rdc_restraint_struct>::iterator
        ...
        to = topo.rdc_restraints().end();
      for(; it!=to; ++it, ++k) {
        enumerator += pow(conf.special().rdc.curr[k] - it->R0, 2);
...
// FIXME put the rng in conf.special().rdc.rng ?
#ifdef DEBUG
      ...
      const math::VArray old_pos(conf.special().rdc.MFpoint);
      const math::VArray old_vel(conf.special().rdc.MFpointVel);

      vector<double> kT_over_m(number_hvectors);
      for(int h=0; h<number_hvectors; ++h){
        kT_over_m[h] = sqrt(math::k_Boltzmann * sim.param().rdc.temp / conf.special().rdc.MFpointMass[h]);
      }
      ...
      if(fabs(gdt) > 0.05){
        ...
        if(sim.steps() == 0) { // and we do all this to make sure the stochastic integral is initialized
          ...
          for (int h=0; h<number_hvectors; ++h){
            ...
            rng->stddev(sd);
            conf.special().rdc.stochastic_integral_mf(h) = rng->get_gaussian_vec();
          }
        }
        ...
        if(sim.steps() == 0){ // and we do all this to make sure the stochastic integral is initialized
          for (int h=0; h<number_hvectors; ++h){
            if (gamma < math::epsilon) {
              conf.special().rdc.stochastic_integral_mf(h) = 0.0;
            }
            else {
              const double emdth = exp(-gdth);
              const double emdt  = emdth * emdth;
      ...
      for(int h=0; h<number_hvectors; ++h){
        ...
        // Update the velocity
        const math::Vec svh = conf.special().rdc.stochastic_integral_mf(h) * c5 + vrand2; // FIXME what is this?
        const double cf = sim.time_step_size() * c2 / conf.special().rdc.MFpointMass[h]; // FIXME ditto
        // Assign vrand1 to the stochastic integrals
        conf.special().rdc.stochastic_integral_mf(h) = vrand1;
        // 2.11.2.2 (2.11.2.3 minus first term of the third line)
        DEBUG(10, "doing stochastic dynamics: velocities (1)")
        conf.special().rdc.MFpointVel[h] = ((conf.special().rdc.MFpointVel[h] - svh) * c1)
            + (force_vectors_hfield[h] * cf)
            + vrand1;

        // Pos1
        DEBUG(10, "doing stochastic dynamics: positions (1)")
        DEBUG(10, "old pos(" << h << ") " << math::v2s(conf.special().rdc.MFpoint[h]))
        conf.special().rdc.MFpoint[h] += conf.special().rdc.MFpointVel[h] * sim.time_step_size() * c6;
        DEBUG(10, "new pos(" << h << ") " << math::v2s(conf.special().rdc.MFpoint[h]))
        ...
        double length_after_displacement = sqrt(pow(conf.special().rdc.MFpoint[h][0],2) +  pow(conf.special().rdc.MFpoint[h][1],2) + pow(conf.special().rdc.MFpoint[h][2],2) );
        DEBUG(10, "length after displacement 1: " << length_after_displacement)
        DEBUG(10, "MFpoint before rescaling: " << math::v2s(conf.special().rdc.MFpoint[h]))
        conf.special().rdc.MFpoint[h] = conf.special().rdc.MFpoint[h] / length_after_displacement;
        DEBUG(10,  "MFpoint afterrescaling: " << math::v2s(conf.special().rdc.MFpoint[h]));

        DEBUG(10, "doing stochastic dynamics: velocities (2)")
        // Velocity correction 2.11.2.24
        const double cinv = 1.0 / (c6 * sim.time_step_size());
        const math::Vec r = conf.special().rdc.MFpoint[h] - old_pos[h];
        conf.special().rdc.MFpointVel[h] = r * cinv;

        DEBUG(10, "doing stochastic dynamics: positions (2)")
        //2.11.2.25
        const math::Vec sxh = conf.special().rdc.stochastic_integral_mf(h) * c9 + vrand4;
        conf.special().rdc.stochastic_integral_mf(h) = vrand3;
        //2.11.2.26
        conf.special().rdc.MFpoint[h] += (vrand3 - sxh);

        // there should be SHAKE here!
        // instead we just rescale, because it's cheap and the system is simple enough as to not require recursion
        DEBUG(10, "SHAKE(rescale) on position (2)")
        length_after_displacement = sqrt(pow(conf.special().rdc.MFpoint[h][0],2) +  pow(conf.special().rdc.MFpoint[h][1],2) + pow(conf.special().rdc.MFpoint[h][2],2) );
        DEBUG(10, "length after displacement 1: " << length_after_displacement)
        DEBUG(10, "MFpoint before rescaling: " << math::v2s(conf.special().rdc.MFpoint[h]))
        conf.special().rdc.MFpoint[h] /=  length_after_displacement;
        DEBUG(10, "MFpoint after rescaling: " << math::v2s(conf.special().rdc.MFpoint[h]))

        DEBUG(10, "SHAKE(rescale) on velocities (1)")
        DEBUG(10, "vel before shake: " << math::v2s(conf.special().rdc.MFpointVel[h]))
        conf.special().rdc.MFpointVel[h] = (conf.special().rdc.MFpoint[h] - old_pos[h]) / sim.time_step_size(); // [nm/ps]
        DEBUG(10, "vel after shake: " << math::v2s(conf.special().rdc.MFpointVel[h]))

        // 'Ekin' of the MF vector
        // E = 0.5 * m * v^2 = 0.25 * m * (v_{n-.5}^2 + v_{n+.5}^2)
        // m[u = g/mol], v[nm/ps]
        conf.special().rdc.Ekin += 0.25 * conf.special().rdc.MFpointMass[h] * (math::abs2(old_vel[h]) + math::abs2(conf.special().rdc.MFpointVel[h])); // [kJ/mol]
        DEBUG(10, "Ekin of (" << h << "): " << (math::abs2(old_vel[h]) + math::abs2(conf.special().rdc.MFpointVel[h]) ) * 0.25 * conf.special().rdc.MFpointMass[h])
    ...
    case simulation::rdc_md: {
      ...
      for(int h=0; h<number_hvectors; ++h) {
        // Save old values
        vold = conf.special().rdc.MFpointVel[h]; // [nm/ps]
        rold = conf.special().rdc.MFpoint[h]; // [nm]
        ...
        conf.special().rdc.MFpointVel[h] += force_vectors_hfield[h] * sim.time_step_size() / conf.special().rdc.MFpointMass[h]; // [nm/ps]
        // Position step r = r + v*\Delta t // r[nm], v[nm/ps], dt[ps]
        conf.special().rdc.MFpoint[h] += conf.special().rdc.MFpointVel[h] * sim.time_step_size(); // [nm]
...
//        conf.special().rdc.MFpoint[h] -= (math::dot(conf.special().rdc.MFpoint[h], rold) - 1) * rold;
		const double discriminant = 4 * pow(math::dot(conf.special().rdc.MFpoint[h], rold),2) - 4 * math::abs2(rold) * (math::abs2(conf.special().rdc.MFpoint[h]) - vector_length_squared);
//        cout << "disc: " << discriminant << endl;
		if (discriminant >= 0.0){// small displacement, we can use SHAKE (something SHAKE-like anyway ...)
        	conf.special().rdc.MFpoint[h] -= (2 * math::dot(conf.special().rdc.MFpoint[h], rold) - sqrt(discriminant)) / (2 * math::abs2(rold)) * rold;
        }else{// larger displacement, we rescale in direction of r(t+\delta t) which is cheap and works for any length of r(t+\delta t)
            conf.special().rdc.MFpoint[h] = vector_length / math::abs(conf.special().rdc.MFpoint[h]) * conf.special().rdc.MFpoint[h];
            DEBUG(10, "something went wrong:  very large displacement of MFpoint (requiring very large corrections)")
        }
//        cout <<"abs: " << math::abs(conf.special().rdc.MFpoint[h]) << endl;

        // calculate the new (constrained) velocities with respect to the constrained position
        conf.special().rdc.MFpointVel[h] = (conf.special().rdc.MFpoint[h] - rold) / sim.time_step_size(); // [nm/ps]

        // 'Ekin' of the MF
        // E = 0.5 * m * v^2 = 0.25 * m * (v_{n-.5}^2 + v_{n+.5}^2)
        // m[u = g/mol], v[nm/ps]
        conf.special().rdc.Ekin += 0.25 * conf.special().rdc.MFpointMass[h] * (math::abs2(vold) + math::abs2(conf.special().rdc.MFpointVel[h])); // [kJ/mol]
      } // Loop over MF
#endif // FINITE_DIFF
...
void _calculate_ah(topology::Topology & topo,
  ...
  const int n_ah = 5;
  const int n_rdc = topo.rdc_restraints().size();
  ...
  if(status == GSL_SUCCESS){ // copy back a values
    for(int h=0; h<n_ah; ++h){
      conf.special().rdc.Tensor[h] = gsl_vector_get(x, h); // [nm]
      DEBUG(15, "a[" << h << "]: " << conf.special().rdc.Tensor[h])
    }
...
void _calculate_forces_tensor_T(topology::Topology & topo,
  ...
  const int number_tensor_components = 5;
  ...
  for(; it!=to; ++it, ++k){
    ...
    double ck[number_tensor_components];
    // Calculate the five tensor factors
    ck[0] = (x*x - z*z)/dij2; // [1]
    ck[1] = (y*y - z*z)/dij2; // [1]
    ck[2] = (2*x*y)/dij2; // [1]
    ...
    conf.special().rdc.curr[k] = 0.0;
    for(int i=0; i<number_tensor_components; ++i) {
      conf.special().rdc.curr[k] += ck[i] * conf.special().rdc.Tensor[i]; // [1]
    } // Loop over tensor components
    // multiply with RDC_max to get current RDC value
    conf.special().rdc.curr[k] *= RDC_max(it) / dij3; // [1/ps]
    ...
    for(int i=0; i<number_tensor_components; ++i) {
      ...
      DEBUG(10, "tensor components(" << i << "): " << conf.special().rdc.Tensor[i])
      DEBUG(10, "forces on the tensor(" << i << "): " << force_array_tensor[i])
      DEBUG(10, "velocity of tensor component" << i << "): " << conf.special().rdc.TensorVel[i])
    } // Loop over tensor components
  } // Loop over RDCs
...
void _calculate_forces_atoms_T(topology::Topology & topo,
  ...
  for(; it!=to; ++it, ++k){
    ...
    math::VArray dQ_over_dr(number_tensor_components);
    ...
    for(int h=0; h<number_tensor_components; ++h) {
      force_tensor_comp +=  conf.special().rdc.Tensor[h] * (-3 * ck[h] * rij + dQ_over_dr[h]); // [nm]
    }
...
int _calculate_interactions_tensor(topology::Topology & topo,
  ...
  switch(sim.param().rdc.method) {
    case simulation::rdc_em: {
      ...
      DEBUG(8, "a1-5: " << conf.special().rdc.Tensor[0] << ", " << conf.special().rdc.Tensor[1] << ", " << conf.special().rdc.Tensor[2] << ", " << conf.special().rdc.Tensor[3] << ", " << conf.special().rdc.Tensor[4])
      ...
      double Q = sqrt(enumerator / denominator);
      cout << "SCRIPT " << conf.special().rdc.Tensor[0] << " " << conf.special().rdc.Tensor[1] << " " << conf.special().rdc.Tensor[2] << " " << conf.special().rdc.Tensor[3] << " " << conf.special().rdc.Tensor[4] << " " << Q << endl;
#endif
    ...
    case simulation::rdc_sd: {
...
#ifndef FINITE_DIFF
      if(sim.steps() == 0 && abs(conf.special().rdc.Tensor[0]+2) < 1.e-10) { // init clm to sensible values in the first cycle, if user didn't input values her- or himself.
        _calculate_ah<B>(topo, conf, sim);
      }
#endif // FINITE_DIFF
      ...
      vector<double> force_array_tensor(number_tensor_components, 0.0);
      ...
      // Save old velocities and positions
      vector<double> old_Tensor(conf.special().rdc.Tensor);
      vector<double> old_TensorVel(conf.special().rdc.TensorVel);

      // Calculate SD coefficients
      const double gdt = gamma * sim.time_step_size();
      const double gdth = gdt * 0.5;

      vector<double> kT_over_m(number_tensor_components);
      for (int i=0; i<number_tensor_components; ++i){
         kT_over_m[i] = sqrt(math::k_Boltzmann * sim.param().rdc.temp / conf.special().rdc.TensorMass[i]);
      }
      ...
      if(fabs(gdt) > 0.05){
        ...
        if(sim.steps() == 0){ // and we do all this to make sure the stochastic integral is initialized
          for (int h=0; h<number_tensor_components; ++h){
            const double sd = kT_over_m[h] / gamma * sqrt(cdth);
            rng->stddev(sd);
            conf.special().rdc.stochastic_integral_t[h] = rng->get_gauss();
          }
        }
        ...
        if(sim.steps() == 0) { // and we do all this to make sure the stochastic integral is initialized
          for (int h=0; h<number_tensor_components; ++h){
            if (gamma < math::epsilon) {
              conf.special().rdc.stochastic_integral_t[h] = 0.0;
            }
            else {
              const double emdth = exp(-gdth);
              const double emdt  = emdth * emdth;
      ...
      for(int h=0; h<number_tensor_components; ++h){
        ...
        const double svh = conf.special().rdc.stochastic_integral_t[h] * c5 + vrand2; //save the old integral // FIXME really?
        const double cf = sim.time_step_size() * c2 / conf.special().rdc.TensorMass[h]; // FIXME what is this
        // Assign vrand1 to the stochastic integrals
        conf.special().rdc.stochastic_integral_t[h] = vrand1;
        // 2.11.2.2 (2.11.2.3 minus first term of the third line)
        conf.special().rdc.TensorVel[h] = ((conf.special().rdc.TensorVel[h] - svh) * c1)
            + (force_array_tensor[h] * cf)
            + vrand1;

        // Pos1
        DEBUG(10, "doing stochastic dynamics: positions (1)")
        DEBUG(10, "old pos(" << h << ") " << conf.special().rdc.Tensor[h])
        conf.special().rdc.Tensor[h] += conf.special().rdc.TensorVel[h] * sim.time_step_size() * c6;
        DEBUG(10, "new pos(" << h << ") " << conf.special().rdc.Tensor[h])

        // Vel2
        DEBUG(10, "doing stochastic dynamics: velocities (2)")
        // Velocity correction 2.11.2.24
        const double cinv = 1.0 / (c6 * sim.time_step_size());
        const double r = conf.special().rdc.Tensor[h] -  old_Tensor[h];
        conf.special().rdc.TensorVel[h] = r * cinv;

        // Pos2
        DEBUG(10, "doing stochastic dynamics: positions (2)")
        //2.11.2.25
        const double sxh = conf.special().rdc.stochastic_integral_t[h] * c9 + vrand4;
        conf.special().rdc.stochastic_integral_t[h] = vrand3;
        //2.11.2.26
        conf.special().rdc.Tensor[h] += (vrand3 - sxh);
        ...
        // E = 0.5 * m * v^2 = 0.25 * m * (v_{n-.5}^2 + v_{n+.5}^2)
        // m[u = g/mol], v[nm/ps]
        conf.special().rdc.Ekin += 0.25 * conf.special().rdc.TensorMass[h] * ( pow(old_TensorVel[h],2) + pow(conf.special().rdc.TensorVel[h],2) ); // [kJ/mol]

      } // Loop over tensor components (5)
#endif // FINITE_DIFF
      DEBUG(8, "a1-5: " << conf.special().rdc.Tensor[0] << ", " << conf.special().rdc.Tensor[1] << ", " << conf.special().rdc.Tensor[2] << ", " << conf.special().rdc.Tensor[3] << ", " << conf.special().rdc.Tensor[4])
      DEBUG(8, "Ekin of tensor: " << conf.special().rdc.Ekin)

      for (int h=0; h<number_tensor_components; ++h){
        DEBUG(12, "stochastic integral (= vrand3) " << h << ": " << conf.special().rdc.stochastic_integral_t[h])
      }
      ...
      double Q = sqrt(enumerator / denominator);
      cout << "SCRIPT " << conf.special().rdc.Tensor[0] << " " << conf.special().rdc.Tensor[1] << " " << conf.special().rdc.Tensor[2] << " " << conf.special().rdc.Tensor[3] << " " << conf.special().rdc.Tensor[4] << " " <<  conf.special().rdc.Ekin << " " << Q << endl;
#endif
    ...
    case simulation::rdc_md: {
      ...
      for(int h=0; h<number_tensor_components; ++h){
        ...
        double vold = conf.special().rdc.TensorVel[h]; // [nm/ps]

        // Velocity step v = v + f*dt/m // f[kJ/(nm*mol)], dt[ps], m[u]
        // 1u = M_u/N_A = g/(6*10^23) && M_u = 1g/mol && N_A = 6*10^23 1/mol
        // but slightly handwavy we can use u = g/mol
        conf.special().rdc.TensorVel[h] += force_array_tensor[h] * sim.time_step_size() / conf.special().rdc.TensorMass[h]; // [nm/ps]
        // Position step r = r + v*dt // r[nm], v[nm/ps], dt[ps]
        conf.special().rdc.Tensor[h] += conf.special().rdc.TensorVel[h] * sim.time_step_size(); // [nm]
        ...
        // E = 0.5 * m * v^2 = 0.25 * m * (v_{n-.5}^2 + v_{n+.5}^2)
        // m[u = g/mol], v[nm/ps]
        conf.special().rdc.Ekin += 0.25 * conf.special().rdc.TensorMass[h] * (pow(vold,2) + pow(conf.special().rdc.TensorVel[h],2)); // [kJ/mol]
      } // Loop over tensor components
#endif // FINITE_DIFF
      DEBUG(8, "a1-5: " << conf.special().rdc.Tensor[0] << ", " << conf.special().rdc.Tensor[1] << ", " << conf.special().rdc.Tensor[2] << ", " << conf.special().rdc.Tensor[3] << ", " << conf.special().rdc.Tensor[4])
      DEBUG(8, "Ekin of tensor: " << conf.special().rdc.Ekin)
...
void _calculate_forces_clm_SH(topology::Topology & topo,
  ...
  for(; it!=to; ++it, ++k){
    ...
    for(int i=0; i<grid_number_of_points; i++){
      ...
      conf.special().rdc.curr[k] += RDC_sampled * g[i] * grid_weight[i]; // [1/ps]
...
void _calculate_forces_atoms_SH(topology::Topology & topo,
  ...
  for(int i=0; i<grid_number_of_points; i++){g[i]=0.0;} // init
  for(int m=-l; m<=l; m++){
    const double c = conf.special().rdc.clm[l+m]; // [nm] // pseudo nm ...
    for(int i=0; i<grid_number_of_points; i++){
...
//        DEBUG(15, num_integral << " (updated)")
  ...
  if(status == GSL_SUCCESS){ // copy back c values
    for(int lm=0; lm<n_clm; ++lm){
      conf.special().rdc.clm[lm] = gsl_vector_get(x, lm); // [nm]
      DEBUG(15, "c[" << lm << "]: " << conf.special().rdc.clm[lm])
    }
  }else if(status == GSL_EDOM){
    // ignore singular matrices which could very rarely occur
    // do not update c_lm
...
int _calculate_interactions_sh(topology::Topology & topo,
  ...
  switch(sim.param().rdc.method) {
    case simulation::rdc_em: {
      ...
      DEBUG(8, "clm: " << conf.special().rdc.clm[0] << ", " << conf.special().rdc.clm[1] << ", " << conf.special().rdc.clm[2] << ", " << conf.special().rdc.clm[3] << ", " << conf.special().rdc.clm[4])
      ...
      double Q = sqrt(enumerator / denominator);
      cout << "SCRIPT " << conf.special().rdc.clm[0] << " " << conf.special().rdc.clm[1] << " " << conf.special().rdc.clm[2] << " " << conf.special().rdc.clm[3] << " " << conf.special().rdc.clm[4] << " " << Q << endl;
#endif
    ...
    case simulation::rdc_sd: {
...
#ifndef FINITE_DIFF
      if(sim.steps() == 0 && abs(conf.special().rdc.clm[0]+2) < 1.e-10) { // init clm to sensible values in the first cycle, if user didn't input values her- or himself.
        _calculate_clm<B>(topo, conf, sim);
      }
#endif // FINITE_DIFF
      ...
      // Save old velocities and positions
      vector<double> old_clm(conf.special().rdc.clm);
      vector<double> old_clmVel(conf.special().rdc.clmVel);
      ...
      for (int lm=0; lm<n_clm; ++lm){
         kT_over_m[lm] = sqrt(math::k_Boltzmann * sim.param().rdc.temp / conf.special().rdc.clmMass[lm]);
      }
      ...
      if(fabs(gdt) > 0.05){
        ...
        if(sim.steps() == 0){ // and we do all this to make sure the stochastic integral is initialized
          for (int lm=0; lm<n_clm; ++lm){
            ...
            rng->stddev(sd);
            conf.special().rdc.stochastic_integral_sh[lm] = rng->get_gauss();
          }
        }
      } // calculate coefficients
      else {
        ...
        if(sim.steps() == 0) { // and we do all this to make sure the stochastic integral is initialized
          for (int lm=0; lm<n_clm; ++lm){
            if (gamma < math::epsilon) {
              conf.special().rdc.stochastic_integral_sh[lm] = 0.0;
            }
            else {
              const double emdth = exp(-gdth);
              const double emdt  = emdth * emdth;
      ...
      for(int lm=0; lm<n_clm; ++lm){
        ...
        // Update the velocity
        // and if we wrote to conf.old it would be much simpler
        const double svh = conf.special().rdc.stochastic_integral_sh[lm] * c5 + vrand2; // save the old integral // FIXME really?
        const double cf = sim.time_step_size() * c2 / conf.special().rdc.clmMass[lm]; // FIXME what is this
        // Assign vrand1 to the stochastic integrals
        conf.special().rdc.stochastic_integral_sh[lm] = vrand1;
        // 2.11.2.2 (2.11.2.3 minus first term of the third line)
        conf.special().rdc.clmVel[lm] = ((conf.special().rdc.clmVel[lm] - svh) * c1)
            + (force_array_clm[lm] * cf)
            + vrand1;

        // Pos1
        DEBUG(10, "doing stochastic dynamics: positions (1)")
        DEBUG(10, "old pos(" << lm << ") " << conf.special().rdc.clm[lm])
        conf.special().rdc.clm[lm] += conf.special().rdc.clmVel[lm] * sim.time_step_size() * c6;
        DEBUG(10, "new pos(" << lm << ") " << conf.special().rdc.clm[lm])

        // Vel2
        DEBUG(10, "doing stochastic dynamics: velocities (2)")
        // Velocity correction 2.11.2.24
        const double cinv = 1.0 / (c6 * sim.time_step_size());
        const double r = conf.special().rdc.clm[lm] -  old_clm[lm];
        conf.special().rdc.clmVel[lm] = r * cinv;

        // Pos2
        DEBUG(10, "doing stochastic dynamics: positions (2)")
        //2.11.2.25
        const double sxh = conf.special().rdc.stochastic_integral_sh[lm] * c9 + vrand4;
        conf.special().rdc.stochastic_integral_sh[lm] = vrand3;
        //2.11.2.26
        conf.special().rdc.clm[lm] += (vrand3 - sxh);
        ...
        // E = 0.5 * m * v^2 = 0.25 * m * (v_{n-.5}^2 + v_{n+.5}^2)
        // m[u = g/mol], v[nm/ps]
        conf.special().rdc.Ekin += 0.25 * conf.special().rdc.clmMass[lm] * ( pow(old_clmVel[lm],2) + pow(conf.special().rdc.clmVel[lm],2) ); // [kJ/mol]

      } // Loop over clm
#endif // FINITE_DIFF

      if(pow(conf.special().rdc.clm[0],2) + pow(conf.special().rdc.clm[1],2) + pow(conf.special().rdc.clm[2],2) + pow(conf.special().rdc.clm[3],2) + pow(conf.special().rdc.clm[4],2) > 1.0){
        DEBUG(10, "The norm of the weight function g is exceeding 1.  Please change T, m or K to avoid this.")
      }

      DEBUG(8, "clm: " << conf.special().rdc.clm[0] << ", " << conf.special().rdc.clm[1] << ", " << conf.special().rdc.clm[2] << ", " << conf.special().rdc.clm[3] << ", " << conf.special().rdc.clm[4])
      DEBUG(8, "Ekin of clm: " << conf.special().rdc.Ekin)

      for (int lm=0; lm<n_clm; ++lm){
        DEBUG(12, "stochastic integral (= vrand3) " << lm << ": " << conf.special().rdc.stochastic_integral_sh[lm])
      }
      ...
      double Q = sqrt(enumerator / denominator);
      cout << "SCRIPT " << conf.special().rdc.clm[0] << " " << conf.special().rdc.clm[1] << " " << conf.special().rdc.clm[2] << " " << conf.special().rdc.clm[3] << " " << conf.special().rdc.clm[4] << " " <<  conf.special().rdc.Ekin << " " << Q << endl;
#endif
    ...
    case simulation::rdc_md: {
      ...
      for(int lm=0; lm<n_clm; ++lm){
        // clm move in 1D
        double vold = conf.special().rdc.clmVel[lm]; // [nm/ps]

        // Velocity step v = v + f*dt/m // f[kJ/(nm*mol)], dt[ps], m[u]
        // 1u = M_u/N_A = g/(6*10^23) && M_u = 1g/mol && N_A = 6*10^23 1/mol
        // but slightly handwavy we can use u = g/mol
        conf.special().rdc.clmVel[lm] += force_array_clm[lm] * sim.time_step_size() / conf.special().rdc.clmMass[lm]; // [nm/ps]
        DEBUG(12, "force, dt, m: " << force_array_clm[lm] << ", " << sim.time_step_size() << ", " << conf.special().rdc.clmMass[lm] << " :: " << force_array_clm[lm] * sim.time_step_size() / conf.special().rdc.clmMass[lm])
        // Position step r = r + v*dt // r[nm], v[nm/ps], dt[ps]
        conf.special().rdc.clm[lm] += conf.special().rdc.clmVel[lm] * sim.time_step_size(); // [nm]
        DEBUG(12, "clm, vel[" << lm << "]: " << conf.special().rdc.clm[lm] << ", " << conf.special().rdc.clmVel[lm])
        ...
        // E = 0.5 * m * v^2 = 0.25 * m * (v_{n-.5}^2 + v_{n+.5}^2)
        // m[u = g/mol], v[nm/ps]
        conf.special().rdc.Ekin += 0.25 * conf.special().rdc.clmMass[lm] * (pow(vold,2) + pow(conf.special().rdc.clmVel[lm],2)); // [kJ/mol]
      } // loop over clm
#endif // FINITE_DIFF
      ...
      vector<topology::rdc_restraint_struct>::iterator
        ...
        to = topo.rdc_restraints().end();
      for(; it!=to; ++it, ++k) {
        cout << conf.special().rdc.curr[k] << " " << it->R0 << endl;
        enumerator += pow(conf.special().rdc.curr[k] - it->R0, 2);
        denominator += pow(it->R0, 2);
      }
      double Q = sqrt(enumerator / denominator);
