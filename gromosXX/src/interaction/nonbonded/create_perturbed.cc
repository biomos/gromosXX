	...
	case math::vacuum:
	  {
	    ...
	    _select_cutoff<math::vacuum, math::no_virial>
	      (ff, topo, sim, conf, it, quiet);
	    break;
	  }
	case math::rectangular:
	  {
	    _select_virial<math::rectangular>
	      (ff, topo, sim, conf, it, quiet);
	    break;
	  }
	case math::triclinic:
	  {
	    _select_virial<math::triclinic>
	      (ff, topo, sim, conf, it, quiet);
	    break;
	  }
	default:
			   ...
			   bool quiet)
{
  ...
  switch(sim.param().pcouple.virial){
    case math::no_virial:
      {
	_select_cutoff<t_boundary, math::no_virial>
	  (ff, topo, sim, conf, it, quiet);
	break;
      }
    case math::atomic_virial:
      {
	_select_cutoff<t_boundary, math::atomic_virial>
	  (ff, topo, sim, conf, it, quiet);
	break;
      }
    case math::molecular_virial:
      {
	_select_cutoff<t_boundary, math::molecular_virial>
	  (ff, topo, sim, conf, it, quiet);
	break;
      }
    default:
			   ...
			   bool quiet)
{
  ...
  DEBUG(9, "\t\tscaling : " << sim.param().perturbation.scaling);
