  ...
  : m_periodicity(periodicity)
{
  ...
  // K direction
  // L x M
  math::Vec cr = math::cross(m_periodicity.box()(1), 
			     m_periodicity.box()(2));
  // unit (L x M)
    ...
    m_outside(false)
{
  ...
  DEBUG(11, "cell: " << m_cell[0] << "\t" << m_cell[1] << "\t" << m_cell[2]);
  ...
  if (k > m_grid.m_mask.upper_k() || 
    ...
    m_eol = true;
    DEBUG(12, "returning from constructor (k) -- empty");
    return;
  }
  ...
  if (l > m_grid.m_mask.upper_l(k) || 
    ...
    DEBUG(13, "++ from constructor (l)");
    m_eol = ! (++(*this));
    DEBUG(13, "returning from l with m_eol=" << m_eol);
    return;
  }
  ...
  if (m > m_grid.m_mask.upper_m(k, l) || 
    ...
    DEBUG(13, "++ from constructor (m)");
    m_eol = ! (++(*this));
    DEBUG(13, "returning from m with m_eol=" << m_eol);
    return;
  }
  ...
  if (it == to){
    DEBUG(13, "empty beginning");
    --it;
    DEBUG(13, "++ from constructor (empty beginning)");
    m_eol = ! (++(*this));
    return;
  }
}
...
::operator++()
{
  ...
  while(it == to){
	...
	if (l + m_cell[1] < 0) {
      ...
      if (m + m_cell[2] < 0){
	m = - m_cell[2];
	DEBUG(13, "corrected m = " << m);
      }
      
    }
    // m is ok
    DEBUG(12, "trying for chargegroups in " << k + m_cell[0] 
	  << " | " << l + m_cell[1] << " | " << m + m_cell[2]);
    DEBUG(12, "with the k = " << k 
	  << " l = " << l << " m = " << m);
  ...
  DEBUG(10, "found cg " << *it << " in " << k+m_cell[0] << " | " << l+m_cell[1]
	<< " | " << m+m_cell[2]);
