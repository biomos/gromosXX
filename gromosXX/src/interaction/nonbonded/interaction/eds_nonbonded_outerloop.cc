...
{  
  ...
  for(i=0; i < size_i; ++i){
	...
	++j_it){
      ...
      DEBUG(1, "\tperturbed nonbonded_interaction: i " << i << " j " << *j_it);
      ...
      innerloop.eds_lj_crf_innerloop(topo, conf, i, *j_it, storage, periodicity, sim);
    }
...
{  
  ...
  // perturbed innerloop
  Eds_Nonbonded_Innerloop<t_interaction_spec, t_perturbation_details> p_innerloop(m_param);
...
{  
  ...
  Eds_Nonbonded_Innerloop<t_interaction_spec, t_perturbation_details> innerloop_p(m_param);
