...
update(t_simulation &sim, t_nonbonded_interaction & nonbonded_interaction)
{
  ...
  Chargegroup_Grid<t_simulation> a_grid(sim.system().periodicity(), 0.5*m_cutoff_short, m_cutoff_long);
  ...
  for(the_cell[0]=0; the_cell[0] < num_cells[0]; ++the_cell[0]){
	...
	DEBUG(8, "the few cg's per grid point: " 
	   ...
	   int cell[3])
{
  ...
  for(int pc=start; pc <= end; ++pc){
    ...
    DEBUG(11, "shift cell: " << grid.shift(pc).cell[0] << " / "
	  << grid.shift(pc).cell[1] << " / " << grid.shift(pc).cell[2]);
    
    int our_cell[3];
    for(int d=0; d<3; ++d)
      our_cell[d] = cell[d] + grid.shift(pc).cell[d];

    // get an iterator over the mask
    Cell_Cell_Iterator<t_simulation, periodic> it(grid, our_cell);
    if (it.eol()) {
      DEBUG(8, "immediately out"); 
      continue;
    }
	...
	cg1 += (*cg_it <= *it) ? *cg_it : *it;
	cg2 += (*cg_it <= *it) ? *it : *cg_it;
