...
__device__ void cudakernel::last_warp_reduce(volatile T *sdata, unsigned int tid) {
...
T cudakernel::calc_sum(unsigned int num_input_elements, T* device_input, T* device_output) {
    ...
    num_blocks = (num_input_elements/num_threads_per_block)/num_elements_per_thread+1;
    unsigned int size = num_input_elements;
    //host_output = (T*) calloc(1,sizeof(T));
    dim3 gridDim(num_blocks,1);
    dim3 blockDim(num_threads_per_block,1);

    const size_t dynamic_shared_memory = num_threads_per_block * sizeof(T);
    while (true) {
        reduce<num_threads_per_block><<< gridDim, blockDim, dynamic_shared_memory >>>(device_input, device_output, size);
        size = num_blocks;
