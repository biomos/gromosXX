...
// for FFTW
template<>
configuration::GenericMesh<configuration::complex_number>::GenericMesh(unsigned int x, unsigned int y, unsigned int z) :
m_x(x), m_y(y), m_z(z), m_volume(x*y*z) {
  ...
  m_mesh = (configuration::complex_number*) fftw_malloc(m_volume * sizeof(fftw_complex));
...
template<typename complex_type>
configuration::GenericMesh<complex_type>::GenericMesh(unsigned int x, unsigned int y, unsigned int z) :
m_x(x), m_y(y), m_z(z), m_volume(x*y*z) {
...
template<>
configuration::GenericMesh<configuration::complex_number>::~GenericMesh() {
  // destory the plans
...
// this function is used for all other types.
template<typename complex_type>
configuration::GenericMesh<complex_type>::~GenericMesh() {
  if (m_mesh != NULL)
    delete m_mesh;
}

template<>
configuration::GenericMesh<configuration::complex_number>::GenericMesh(const GenericMesh<configuration::complex_number> & mesh) {
  m_x = mesh.x();
  m_y = mesh.y();
  m_z = mesh.z();
...
template<typename complex_type>
configuration::GenericMesh<complex_type>::GenericMesh(const GenericMesh<complex_type> & mesh) {
  m_x = mesh.x();
  m_y = mesh.y();
  m_z = mesh.z();
  m_volume = m_x * m_y * m_z;
...
template<>
void configuration::GenericMesh<configuration::complex_number>::resize(unsigned int x, unsigned int y, unsigned int z){
  m_x = x;
  m_y = y;
  m_z = z;
  m_volume = m_x * m_y * m_z;
...
template<typename complex_type>
void configuration::GenericMesh<complex_type>::resize(unsigned int x, unsigned int y, unsigned int z){
  m_x = x;
  m_y = y;
  m_z = z;
  m_volume = m_x * m_y * m_z;
...
template<>
void configuration::GenericMesh<configuration::complex_number>::fft(configuration::GenericMesh<configuration::complex_number>::fft_type type) {
  switch (type) {
    case configuration::GenericMesh<configuration::complex_number>::fft_forward :
              fftw_execute(plan_forward);
      break;
    case configuration::GenericMesh<configuration::complex_number>::fft_backward :
...
template<typename complex_type>
void configuration::GenericMesh<complex_type>::fft(configuration::GenericMesh<complex_type>::fft_type type) {
