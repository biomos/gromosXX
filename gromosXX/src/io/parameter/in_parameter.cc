...
void io::In_Parameter::read(simulation::Parameter &param,
  ...
  read_MULTIAEDS(param); // needs to be called after EDS
...
/**
* @section MULTIAEDS MULTIAEDS block
* @snippet snippets/snippets.cc MULTIAEDS

*/
void io::In_Parameter::read_MULTIAEDS(simulation::Parameter & param,
  std::ostream & os) {
  DEBUG(8, "reading MULTIAEDS");

  std::stringstream exampleblock;
  // lines starting with 'exampleblock<<"' and ending with '\n";' (spaces don't matter)
  // will be used to generate snippets that can be included in the doxygen doc;
  // the first line should be the blockname and is used as snippet tag
  exampleblock << "MULTIAEDS\n";
  exampleblock << "# MULTIAEDS  0,1\n";
  exampleblock << "#              0: no multi-site accelerated enveloping distribution sampling (A-EDS) [default]\n";
  exampleblock << "#              1: multi-site accelerated enveloping distribution sampling, with fixed parameters\n";
  exampleblock << "# NUMSITES  >1  : number of AEDS sites\n";
  exampleblock << "# NUMSTATES >1  : number of states for each site \n";
  exampleblock << "# EMAX          : A-EDS parameter Emax for each site\n";
  exampleblock << "# EMIN          : A-EDS parameter Emin\n";
  exampleblock << "# EIR(NUMSTATES): energy offsets for states\n";
  exampleblock << "#\n";
  exampleblock << "# MULTIAEDS\n";
  exampleblock << "  1\n";
  exampleblock << "# NUMSITES\n";
  exampleblock << "  3\n";
  exampleblock << "# SITE 1\n";
  exampleblock << "# NUMSTATES EMAX EMIN\n";
  exampleblock << "  2         10   -50\n";
  exampleblock << "# EIR\n";
  exampleblock << "  0  -5\n";
  exampleblock << "# SITE 2\n";
  exampleblock << "# NUMSTATES EMAX EMIN\n";
  exampleblock << "  3         20   -60\n";
  exampleblock << "# EIR\n";
  exampleblock << "  0  -140  -560\n";
  exampleblock << "# SITE 3\n";
  exampleblock << "# NUMSTATES EMAX EMIN\n";
  exampleblock << "  4         30   -70\n";
  exampleblock << "# EIR\n";
  exampleblock << "  0  -74  -90  -180\n";
  exampleblock << "END\n";


  std::string blockname = "MULTIAEDS";
  Block block(blockname, exampleblock.str());

  if (block.read_buffer(m_block[blockname], false) == 0) {
    block_read.insert(blockname);

    block.get_next_parameter("MULTIAEDS", param.eds.multiaeds, "", "0,1");
    block.get_next_parameter("NUMSITES", param.eds.numsites, ">=2", "");

    // initialize vectors for numstates, emax, emin and eir
    param.eds.multnumstates.resize(param.eds.numsites, 0.0);
    param.eds.multemax.resize(param.eds.numsites, 0.0);
    param.eds.multemin.resize(param.eds.numsites, 0.0);
    param.eds.multeir.resize(param.eds.numsites);

    // read numstates, Emax, Emin and Eir for each eds site
    for (unsigned int i = 0; i < param.eds.numsites; i++) {
      std::string idx = io::to_string(i);
      block.get_next_parameter("NUMSTATES[" + idx + "]", param.eds.multnumstates[i], "", "");
      block.get_next_parameter("EMAX[" + idx + "]", param.eds.multemax[i], "", "");
      block.get_next_parameter("EMIN[" + idx + "]", param.eds.multemin[i], "", "");
    ...
    //generate a map with all possible combinations of 2 sites
    //key is the site_state_pair and the value is an index
      ...
      for( int site_j = site_i; site_j < param.eds.numsites + 1; site_j++){ // +1 for the non-eds site
        for( int state_i =0; state_i < param.eds.multnumstates[site_i]; state_i++){
          if( site_i == site_j){
            ...
            param.eds.site_state_pairs[states_i_j] = index;
            DEBUG(7,"site_state_pair: " << site_i << "," << state_i << "," << site_i << "," << state_i << ", index: " << index);
            index++;
            ...
            for( int state_j=0; state_j < param.eds.multnumstates[site_j]; state_j++){
              ...
              param.eds.site_state_pairs[states_i_j] = index;
              DEBUG(7,"site_state_pair: " << site_i << "," << state_i << "," << site_j << "," << state_j << ", index: " << index);
              index++;
              //param.eds.site_state_pairs.push_back(states_i_j);
            }
          }
    ...
    if (param.eds.eds != 1 && param.eds.eds != 2) {
      ...
      io::messages.add("MULTIAEDS block: MULTIAEDS cannot be used in combination with EDS or AEDS.",
                                 "In_Parameter", io::message::error);
    }
    ...
    for (unsigned int i = 0; i < param.eds.numsites; i++) {
      if (param.eds.multemin[i] > param.eds.multemax[i]) {
        std::string idx = io::to_string(i);
        io::messages.add("MULTIAEDS parameter EMIN[" + idx + "] is larger than EMAX[" + idx + "]",
          "In_Parameter", io::message::warning);
      return;
      }
    }
