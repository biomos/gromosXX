/*
 * File:   hamiltonian_simulatedAnnealing_master_eds.cc
 * Author: bschroed
 *
...
re::hamiltonian_simulatedAnnealing_master_eds::hamiltonian_simulatedAnnealing_master_eds(io::Argument _args,
        ...
        hamiltonian_simulatedAnnealing_base_eds(_args, cont, globalThreadID,  replicaGraphMPIControl, replica_mpi_control),
        replica_exchange_master_interface(_args, cont, globalThreadID, replicaGraphMPIControl, replica_mpi_control)
{
    
    #ifdef XXMPI
    DEBUG(2,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":Constructor:\t START");  
    DEBUG(3,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":Constructor:\t Replicas: "<<replicaGraphMPIControl.numberOfReplicas);  
    DEBUG(3,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":Constructor:\t ReplicasOLD: "<<repParams.num_l);  
    DEBUG(3,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":Constructor:\t ReplicasMASTER: "<< replicaGraphMPIControl.masterID);  
    //initialize data of replicas    
    replicaData.resize(replicaGraphMPIControl.numberOfReplicas);      
    DEBUG(3,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":Constructor:\t replicaDatasize\t "<< replicaData.size());
    //DEBUG(4,"hamiltonian_simulatedAnnealing_master_eds "<< rank <<":Constructor:\t reeds- lambda\t "<< replica->sim.param().reeds.num_l);
    //DEBUG(4,"hamiltonian_simulatedAnnealing_master_eds "<< rank <<":Constructor:\t eds \t "<< replica->sim.param().eds.s.size());

    //initialize bookkeeping data structure
    coordIDPositionsVector.resize(replicaGraphMPIControl.numberOfReplicas);
    DEBUG(3,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":Constructor:\t coordIDPositionsVectorsize\t "<< coordIDPositionsVector.size());
    ...
    for (int replicaID = 0; replicaID<  replicaData.size(); ++replicaID) {
        ...
        DEBUG(5,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":Constructor:\t Init Replicas ID"<<replicaID<<"\t "<< repParams.temperature[replicaID]);
        replicaData[replicaID].l = repParams.lambda[replicaID];
        replicaData[replicaID].dt = repParams.dt[replicaID];
        DEBUG(5,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":Constructor:\t Init Replicas eds_param"<<replicaID<<"\t "<< reedsParam.eds_para[0].numstates);
        replicaData[replicaID].Vi.assign(reedsParam.eds_para[0].numstates,0);
    }
    //DEBUG(2,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":Constructor:\t SIMID "<< simulationID <<"\n");
    DEBUG(2,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":Constructor:\t DONE\n");
    #else
        throw "Cannot initialize hamiltonian_simulatedAnnealing_master_eds without MPI!";
    #endif

}

void re::hamiltonian_simulatedAnnealing_master_eds::receive_from_all_slaves() {
  #ifdef XXMPI
  DEBUG(2,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":receive_from_all_slaves:\t START\n");
  ...
  DEBUG(4,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":receive_from_all_slaves:\t receive from slaves");
  DEBUG(4, "hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":receive_from_all_slaves:\t numReps: "<< replicaGraphMPIControl().numberOfReplicas)
  for (unsigned int slaveReplicaID = 0; slaveReplicaID < replicaGraphMPIControl().numberOfReplicas; ++slaveReplicaID) {
    if (slaveReplicaID != replicaGraphMPIControl().masterID) {
        DEBUG(2,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":receive_from_all_slaves:\t \t out of rank ID: " << replicaGraphMPIControl().masterID);
        MPI_Recv(&info, 1, MPI_REPINFO, slaveReplicaID, REPINFO, replicaGraphMPIControl().comm, &status);
        DEBUG(2,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":receive_from_all_slaves:\t \t ID: " << replicaGraphMPIControl().masterID);
        ...
        DEBUG(4,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":receive_from_all_slaves:\t REP:" <<slaveReplicaID<< " EpotTot: "<< replicaData[slaveReplicaID].epot);
        ...
        for(unsigned int s=0;s< replicaData[slaveReplicaID].Vi.size(); s++){
            DEBUG(4,"hamiltonian_simulatedAnnealing_master_eds "<< replicaGraphMPIControl().masterID <<":receive_from_all_slaves:\t "<< s << " En: "<< replicaData[slaveReplicaID].Vi[s]);
        }        
    }    
  }

  // write all information from master node to data structure
  DEBUG(2,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":receive_from_all_slaves:\t  t\twrite_own data");
  ...
  DEBUG(4,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":receive_from_all_slaves:\t Master:\n" << "time used for receiving all messages: " << MPI_Wtime() - start
            << " seconds");
  DEBUG(2,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":receive_from_all_slaves:\t DONE")
  #else
    throw "Cannot use receive_from_all_slaves from hamiltonian_simulatedAnnealing_master_eds without MPI!";
  #endif
}

int re::hamiltonian_simulatedAnnealing_master_eds::getSValPrecision(double minLambda){
...
void re::hamiltonian_simulatedAnnealing_master_eds::write() {
    DEBUG(2,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":write:\t START");
    DEBUG(2,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":write:\t svalPrecision "<<svalPrecision);
    ...
    for (unsigned int treplicaID = 0; treplicaID < replicaGraphMPIControl().numberOfReplicas; ++treplicaID) {
      ...
      DEBUG(2,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":write:\t s_val raus! "<<replicaData[treplicaID].l);
    ...
    for(unsigned int s=0;s<reedsParam.eds_para[0].numstates; s++){
        DEBUG(2, "hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":write:\t WRITE out POTS " <<s<< "\t" << replicaData[treplicaID].Vi[s]);
        repOut   << std::setw(18) << std::min(replicaData[treplicaID].Vi[s], 10000000.0);//Output potential energies for each state
    }
      repOut << std::endl;
    }
    DEBUG(2,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":write:\t DONE");
}

void re::hamiltonian_simulatedAnnealing_master_eds::init_repOut_stat_file() {
    DEBUG(2,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":init_repOut_stat_file:\t START");

    repOut.open(repdatName.c_str());
    DEBUG(2,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":init_repOut_stat_file:\t repdat file open ");
    ...
    DEBUG(4,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":init_repOut_stat_file:\t set precision ");
    ...
    repOut << "\n";
    repOut.flush();
    DEBUG(2,"hamiltonian_simulatedAnnealing_master_eds "<< globalThreadID <<":init_repOut_stat_file:\t DONE");
}
