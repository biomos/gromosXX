        ...
        std::vector<double> &memory)
{
  ...
  std::vector<BS_Potential *>::iterator pot_i = potentials.begin(),
                                        pot_end = potentials.end();
  for (; pot_i != pot_end; pot_i++){
    if ((*pot_i)->potentialType == type && (*pot_i)->id == id) {
...
void util::BS_Subspace::transformCoordinates(configuration::Configuration& conf, 
  ...
  std::vector<BS_Dimension> coord;
  std::vector<BS_Dimension>::iterator it, to;
...
void util::BS_Subspace::addPotential(BS_Potential* newPotential){
  ...
  if (newPotential->potentialType == BS_Potential::bs_sphere){
    DEBUG(8, "Subspace: Added new Sphere");
    m_numSpheres++;
  }
  else if (newPotential->potentialType == BS_Potential::bs_stick){
    DEBUG(8, "Subspace: Added new Stick");
    m_numSticks++;
  }
  else {
    io::messages.add("Added unknown type of potential to umbrella",
...
util::BS_Vector util::BS_Subspace::getCenter(int id){
  std::vector<BS_Potential *>::iterator pot_i = potentials.begin(),
                                        pot_end = potentials.end();
  for (; pot_i != pot_end; pot_i++){
    if ((*pot_i)->potentialType == BS_Potential::bs_sphere &&
            (*pot_i)->id == id) {
...
void util::BS_Subspace::getForce(std::vector<double>& force){
  ...
  for (; force_i != force_end; force_i++){
    force.push_back(force_i->value);
  }
}

void util::BS_Subspace::getPeriodicities(std::vector<double> &periodicities){
  periodicities.clear();
  std::vector<util::BS_Coordinate *>::iterator coord_i = m_definition.begin(),
                                          coord_end = m_definition.end();
  for (; coord_i != coord_end; coord_i++){
    periodicities.push_back((*coord_i)->getPeriodicity());
  }
}
...
std::string util::BS_Subspace::traj_str(){
  ...
  if (m_numSpheres > 0) {
    ...
    std::vector<BS_Potential *>::iterator it = potentials.begin(),
            to = potentials.end();
    for (; it != to; it++) {
      if ((*it)->potentialType == BS_Potential::bs_sphere) {
        os << (*it)->traj_str() << "\n";
      }
    }
  }
  ...
  if (m_numSticks > 0) {
    ...
    std::vector<BS_Potential *>::iterator it = potentials.begin(),
            to = potentials.end();
    for (; it != to; it++) {
      if ((*it)->potentialType == BS_Potential::bs_stick) {
        os << (*it)->traj_str() << "\n";
      }
    }
  }
