...
int io::read_input(io::Argument const & args,
    ...
    switch (sim.param().hoomd.processor) {
        case simulation::cpu: sim.proc = boost::shared_ptr<processor::Processor>(new processor::Processor(processor::CPU));
            break;
        case simulation::gpus: sim.proc = boost::shared_ptr<processor::Processor>(new processor::Processor(processor::GPUs));
            break;
        default: break;
    }
#endif
    ...
    if (cont == 1 && rank == 0) {
        ...
        for (int x = 0; x < numReplicas; x++) {
            ...
            if (read_configuration(tmpArgs, topo, conf, sim, os, quiet)) {
                error = 1;
                io::messages.add("\tCould not find coordinate file: " + std::string(it->second) + "\n", io::message::error);
            }
        }
        if (error) {
            return -1;
