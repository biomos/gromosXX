  ...
  for(int z=begin; z < m_grid.Nc + c_ex; z+=stride){
	...
	if (m_grid.p_cell[z][i].i < num_solute_cg){ // self interaction
	  ...
	  for(int a1 = topo.chargegroups()[m_grid.p_cell[z][i].i],
		...
		continue;
	      pairlist.solute_short.push_back(a1, a2);  //changed
	    }
	  }
	}
		...
		for(int a2 = topo.chargegroups()[m_grid.p_cell[z][j].i],
		    ...
		    a2 < a2_to; ++a2) {
                  if (a2 >= num_solute_atoms) {
                    ...
                    // CUDA
                    if (no_cuda)
                      pairlist.solvent_short.push_back(a2, a1);  //changed
#else
                    pairlist.solvent_short.push_back(a2, a1);  //changed
#endif
                  } else
                    pairlist.solute_short.push_back(a2, a1);  //changed
		}
	      }
	    }
	  }
	  ...
	  for(unsigned int m=0; m<m_grid.mask[mask_z].size(); m+=2){
		...
		if (d2 > m_cutoff_short_2){ // LONGRANGE
		  ...
		  for(int a1 = topo.chargegroups()[m_grid.p_cell[i_level][i].i],
			...
			a2 < a2_to; ++a2){
		      pairlist.solute_long.push_back(a1, a2); //changed
		    }
		  }
		}
		else{ // SHORTRANGE
...
#ifdef HAVE_LIBCUKERNEL
                      if (no_cuda)
                        pairlist.solvent_long.push_back(a2, a1);   //changed
#else
                      pairlist.solvent_long.push_back(a2, a1);    //changed
#endif
                    } else
                        pairlist.solute_long.push_back(a2, a1);   //changed
		  }
		}
	      }
	      else{ // SHORTRANGE
		  ...
		  for(int a2 = topo.chargegroups()[p_plane[j].i],
			...
			(topo, pairlist.solute_long, perturbed_pairlist.solute_long,
			 a2, a1, scaled_only))
		      ...
		      pairlist.solvent_long.push_back(a1, a2); //changed
                    else
                      pairlist.solute_long.push_back(a1, a2);  //changed
 ...
 )
{
  if (topo.is_perturbed(a1) || topo.is_eds_perturbed(a1)){
	...
	perturbed_pairlist.push_back(a1, a2); //changed
      else
	pairlist.push_back(a1, a2); //changed
    } // scaling
    else{
      perturbed_pairlist.push_back(a1, a2); //changed
    }
