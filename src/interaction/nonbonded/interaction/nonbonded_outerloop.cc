...
::lj_crf_outerloop(topology::Topology & topo,
       ...
       // Pairlist const & pairlist_solvent, //changed
          Pairlist & pairlist_solute,
          Pairlist & pairlist_solvent,
        Storage & storage,
...
::_lj_crf_outerloop(topology::Topology & topo,
  ...
  unsigned int i;
  for (i = 0; i < end; ++i) {
    for (j_it = pairlist_solute.begin(i),  //changed
            j_to = pairlist_solute.end(i);  //changed
            j_it != j_to;
            ++j_it) {
  ...
  if (sim.param().force.special_loop == simulation::special_loop_spc) { // special solvent loop
    ...
    for (; i < size_i; i += 3) { // use every third pairlist (OW's)
      for (j_it = pairlist_solvent.begin(i),  //changed
              j_to = pairlist_solvent.end(i);  //changed
              j_it != j_to;
...
::electric_field_outerloop(topology::Topology & topo,
        configuration::Configuration & conf,
        simulation::Simulation & sim,
        //changed PairlistContainer const & pairlist,
...
::_electric_field_outerloop(topology::Topology & topo,
  ...
  if (!(sim.steps() % sim.param().pairlist.skip_step)) {
    ...
    for (i = 0; i < end_lr; ++i) {
      // solute longrange
     //changed for (std::vector<unsigned int>::const_iterator
        for (Pairlist::iterator
        j_it = pairlist.solute_long.begin(i), //changed
              j_to = pairlist.solute_long.end(i); //changed
              j_it != j_to; ++j_it){
      ...
      //changed for (std::vector<unsigned int>::const_iterator
        for (Pairlist::iterator
        j_it = pairlist.solvent_long.begin(i), //changed
              j_to = pairlist.solvent_long.end(i); //changed
              j_it != j_to; ++j_it){
        if (!topo.is_polarisable(i) && !topo.is_polarisable(*j_it)) continue;
        math::Vec e_eli_lr, e_elj_lr;
  ...
  while (minfield >= minfield_param) {
    ...
    for (i = 0; i < end; ++i) {
      ...
      //changed for (std::vector<unsigned int>::const_iterator
        for (Pairlist::iterator
        j_it = pairlist.solute_short.begin(i), //changed
              j_to = pairlist.solute_short.end(i); //changed
              j_it != j_to; ++j_it) {
        if (!topo.is_polarisable(i) && !topo.is_polarisable(*j_it)) continue;
      ...
      //changed for (std::vector<unsigned int>::const_iterator
      for (Pairlist::iterator
        j_it = pairlist.solvent_short.begin(i),  //changed
              j_to = pairlist.solvent_short.end(i);  //changed
              j_it != j_to; ++j_it) {
        if (!topo.is_polarisable(i) && !topo.is_polarisable(*j_it)) continue;
        math::Vec e_eli, e_elj;
...
::_ls_real_outerloop(topology::Topology & topo,
  ...
  /*
    ...
    outer index has to be integer...
   */
  // changed std::vector<unsigned int>::const_iterator j_it, j_to;
  Pairlist::iterator j_it, j_to;
...
*/ //changed
  bool calculate_pair =
          std::find(pairlist.solute_short.begin(atom_i),
          pairlist.solute_short.end(atom_i),
          atom_j) != pairlist.solute_short.end(atom_i) || // i-j in solute
          std::find(pairlist.solute_short.begin(atom_j),
          pairlist.solute_short.end(atom_j),
          atom_i) != pairlist.solute_short.end(atom_i) || // j-i in solute
          std::find(pairlist.solvent_short.begin(atom_i),
          pairlist.solvent_short.end(atom_i),
          atom_j) != pairlist.solvent_short .end(atom_i) || // i-j in solvent
          std::find(pairlist.solvent_short.begin(atom_j),
          pairlist.solvent_short.end(atom_j),
          atom_i) != pairlist.solvent_short.end(atom_j); // j-i in solvent
