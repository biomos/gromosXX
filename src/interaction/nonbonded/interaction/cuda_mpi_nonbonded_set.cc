 ...
 * Constructor.
 */
interaction::CUDA_MPI_Nonbonded_Set
::CUDA_MPI_Nonbonded_Set(Pairlist_Algorithm & pairlist_alg, PairlistContainer & pairlist_container,
                Nonbonded_Parameter & param, int rank, int num_threads)
  : Nonbonded_Set_Interface(pairlist_alg, pairlist_container, param, rank, num_threads),
    m_outerloop(param)
{  //changed
}
 ...
 */
int interaction::CUDA_MPI_Nonbonded_Set
			 ...
			 simulation::Simulation & sim)
{
  ...
  const bool pairlist_update = !(sim.steps() % sim.param().pairlist.skip_step); // bool needed
...
int interaction::CUDA_MPI_Nonbonded_Set::update_configuration
(
 ...
 */
int interaction::CUDA_MPI_Nonbonded_Set::calculate_interaction
(
