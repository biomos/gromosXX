 ...
 * Constructor.
 */
interaction::MPI_Nonbonded_Master::MPI_Nonbonded_Master(Pairlist_Algorithm *pa, PairlistContainer *pc)
        ...
        bool quiet) 
{
  ...
  if (sim.param().perturbation.perturbation){
    ...
    // only one set per MPI process
    m_nonbonded_set.push_back(new Nonbonded_Set(*m_pairlist_algorithm, *m_pairlist_container,
            m_parameter, rank, num_threads));
  }
