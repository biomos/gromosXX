...
(
  ...
  lj_parameter_struct const *A_i_lj;
  lj_parameter_struct const *B_i_lj;
  lj_parameter_struct const *A_j_lj;
  lj_parameter_struct const *B_j_lj;
  ...
  if (j < topo.num_solute_atoms() && topo.is_perturbed(j) == true) {
    ...
    switch (t_interaction_spec::interaction_func) {
      ...
      case simulation::lj_crf_2d_func:
        ...
        B_lj = &m_param->lj_parameter(topo.perturbed_solute().atoms()[i].B_IAC(),
                topo.perturbed_solute().atoms()[j].B_IAC());
        A_i_lj = &m_param->lj_parameter(topo.perturbed_solute().atoms()[i].A_IAC(),
                topo.perturbed_solute().atoms()[i].A_IAC());
        A_j_lj = &m_param->lj_parameter(topo.perturbed_solute().atoms()[j].A_IAC(),
                topo.perturbed_solute().atoms()[j].A_IAC());
        B_i_lj = &m_param->lj_parameter(topo.perturbed_solute().atoms()[i].B_IAC(),
                topo.perturbed_solute().atoms()[i].B_IAC());
        B_j_lj = &m_param->lj_parameter(topo.perturbed_solute().atoms()[j].B_IAC(),
                topo.perturbed_solute().atoms()[j].B_IAC());
        break;
...
//        std::cout << "lj_crf_soft_interaction: atom i, j " << i << " " << j << 
//                     "\te_lj: " << e_lj << " e_crf: " << e_crf
//                << " de_lj: " << de_lj << " de_crf: " << de_crf << std::endl;
        ...
        // if nr_lambdas > 0, we apply extended TI 
        // but we only calculate it when we write out the (free) energy trajectory
        if (((sim.steps()  % sim.param().write.free_energy) == 0) && 
          ...
          for (int lam_index = 0; lam_index < sim.param().precalclam.nr_lambdas; ++lam_index){ 
            ...
            DEBUG(8, "precalculated energies for lambda " << lam
      ...
      case simulation::lj_crf_2d_func:
      {
        math::Vec f;
        double f1, f6, f12;
        // use special 2d perturbation only when both i and j are perturbed and
        // belong to different energy groups, use regular function otherwise
        if((j < topo.num_solute_atoms() && topo.is_perturbed(j) == true) && (n1!=n2)){
          double pert2d_lamI_lj, pert2d_lamI_ljs, pert2d_lamI_crf, pert2d_lamI_crfs;
          double pert2d_lamJ_lj, pert2d_lamJ_ljs, pert2d_lamJ_crf, pert2d_lamJ_crfs;
          set_lambda_2d(topo.individual_lambda(simulation::lj_lambda)[n1][n1],
                        topo.individual_lambda(simulation::lj_lambda)[n2][n2],
                        topo.individual_lambda(simulation::lj_softness_lambda)[n1][n1],
                        topo.individual_lambda(simulation::lj_softness_lambda)[n2][n2],
                        topo.individual_lambda(simulation::crf_lambda)[n1][n1],
                        topo.individual_lambda(simulation::crf_lambda)[n2][n2],
                        topo.individual_lambda(simulation::crf_softness_lambda)[n1][n1],
                        topo.individual_lambda(simulation::crf_softness_lambda)[n2][n2],
	                topo.individual_lambda_derivative(simulation::lj_lambda)[n1][n1],
                        topo.individual_lambda_derivative(simulation::lj_lambda)[n2][n2],
                        topo.individual_lambda_derivative(simulation::lj_softness_lambda)[n1][n1],
                        topo.individual_lambda_derivative(simulation::lj_softness_lambda)[n2][n2],
                        topo.individual_lambda_derivative(simulation::crf_lambda)[n1][n1],
                        topo.individual_lambda_derivative(simulation::crf_lambda)[n2][n2],
                        topo.individual_lambda_derivative(simulation::crf_softness_lambda)[n1][n1],
                        topo.individual_lambda_derivative(simulation::crf_softness_lambda)[n2][n2]);

          lj_crf_soft_2d_interaction(r, A_i_lj->c6, A_i_lj->c12,
                B_i_lj->c6, B_i_lj->c12,
                A_j_lj->c6, A_j_lj->c12, B_j_lj->c6, B_j_lj->c12,
                A_qi, B_qi, A_qj, B_qj,
                alpha_lj, alpha_crf,
                f1, f6, f12,
                e_lj, e_crf, de_lj, de_crf,
                pert2d_lamI_lj, pert2d_lamI_ljs, pert2d_lamI_crf, pert2d_lamI_crfs,
                pert2d_lamJ_lj, pert2d_lamJ_ljs, pert2d_lamJ_crf, pert2d_lamJ_crfs);

          storage.perturbed_energy_derivatives.pert2d_lj[n1] += pert2d_lamI_lj;
          storage.perturbed_energy_derivatives.pert2d_ljs[n1] += pert2d_lamI_ljs;
          storage.perturbed_energy_derivatives.pert2d_crf[n1] += pert2d_lamI_crf;
          storage.perturbed_energy_derivatives.pert2d_crfs[n1] += pert2d_lamI_crfs;
          storage.perturbed_energy_derivatives.pert2d_lj[n2] += pert2d_lamJ_lj;
          storage.perturbed_energy_derivatives.pert2d_ljs[n2] += pert2d_lamJ_ljs;
          storage.perturbed_energy_derivatives.pert2d_crf[n2] += pert2d_lamJ_crf;
          storage.perturbed_energy_derivatives.pert2d_crfs[n2] += pert2d_lamJ_crfs;


//          std::cout << "lj_crf_soft_2d_interaction: atom i, j " << i << " " << j <<
//                     "\te_lj: " << e_lj << " e_crf: " << e_crf
//                << " de_lj: " << de_lj << " de_crf: " << de_crf << std::endl;

        } else{
          double pert2d_lj, pert2d_ljs, pert2d_crf, pert2d_crfs;
          set_lambda_2d_other(topo.individual_lambda(simulation::lj_lambda)[n1][n2],
          topo.individual_lambda(simulation::lj_softness_lambda)[n1][n2],
          topo.individual_lambda(simulation::crf_lambda)[n1][n2],
          topo.individual_lambda(simulation::crf_softness_lambda)[n1][n2],
          topo.individual_lambda_derivative(simulation::lj_lambda)[n1][n2],
          ...
          lj_crf_soft_2d_other_interaction(r, A_lj->c6, A_lj->c12,
                B_lj->c6, B_lj->c12,
                A_q, B_q,
                alpha_lj, alpha_crf,
                f1, f6, f12,
                e_lj, e_crf, de_lj, de_crf,
                pert2d_lj, pert2d_ljs, pert2d_crf, pert2d_crfs);

          storage.perturbed_energy_derivatives.pert2d_lj[n1] += pert2d_lj;
          storage.perturbed_energy_derivatives.pert2d_ljs[n1] += pert2d_ljs;
          storage.perturbed_energy_derivatives.pert2d_crf[n1] += pert2d_crf;
          storage.perturbed_energy_derivatives.pert2d_crfs[n1] += pert2d_crfs;


//          std::cout << "2d_lj_crf_soft_interaction: atom i, j " << i << " " << j <<
//                     "\te_lj: " << e_lj << " e_crf: " << e_crf
//                << " de_lj: " << de_lj << " de_crf: " << de_crf << std::endl;
...
(topology::Topology & topo,
  ...
  if (topo.is_perturbed(j) == true) {
    if (t_interaction_spec::interaction_func == simulation::lj_crf_2d_func) {
      A_lj = &m_param->lj_parameter(topo.perturbed_solute().atoms()[i].A_IAC(),
              topo.perturbed_solute().atoms()[j].A_IAC());
      B_lj = &m_param->lj_parameter(topo.perturbed_solute().atoms()[i].B_IAC(),
              topo.perturbed_solute().atoms()[j].B_IAC());
...
//      case simulation::lj_crf_2d_func:
      case simulation::lj_crf_func:
      {
        ...
        if (((sim.steps()  % sim.param().write.free_energy) == 0) &&
          ...
          for (int lam_index = 0; lam_index < sim.param().precalclam.nr_lambdas; ++lam_index){ 
            ...
            DEBUG(8, "perturbed one_four: precalculated energies for lambda " << lam
                   << "\n now starting storage");
            DEBUG(8, "\n  A_e_lj " << A_e_lj << "\n  lambda index " << lam_index <<
      ...
      case simulation::lj_crf_2d_func:
      {
        ...
        if((j < topo.num_solute_atoms() && topo.is_perturbed(j) == true) && (n1!=n2)){
          ...
          lj_crf_soft_2d_interaction(r, A_i_lj->cs6, A_i_lj->cs12,
                B_i_lj->cs6, B_i_lj->cs12,
                A_j_lj->cs6, A_j_lj->cs12, B_j_lj->cs6, B_j_lj->cs12,
                A_qi, B_qi, A_qj, B_qj,
                alpha_lj, alpha_crf,
                f1, f6, f12,
                e_lj, e_crf, de_lj, de_crf,
          ...
          conf.current().perturbed_energy_derivatives.pert2d_lj[n1] += pert2d_lamI_lj;
          conf.current().perturbed_energy_derivatives.pert2d_ljs[n1] += pert2d_lamI_ljs;
          conf.current().perturbed_energy_derivatives.pert2d_crf[n1] += pert2d_lamI_crf;
          conf.current().perturbed_energy_derivatives.pert2d_crfs[n1] += pert2d_lamI_crfs;
          conf.current().perturbed_energy_derivatives.pert2d_lj[n2] += pert2d_lamJ_lj;
          conf.current().perturbed_energy_derivatives.pert2d_ljs[n2] += pert2d_lamJ_ljs;
          conf.current().perturbed_energy_derivatives.pert2d_crf[n2] += pert2d_lamJ_crf;
          conf.current().perturbed_energy_derivatives.pert2d_crfs[n2] += pert2d_lamJ_crfs;
          ...
          conf.current().perturbed_energy_derivatives.pert2d_lj[n1] += pert2d_lj;
          conf.current().perturbed_energy_derivatives.pert2d_ljs[n1] += pert2d_ljs;
          conf.current().perturbed_energy_derivatives.pert2d_crf[n1] += pert2d_crf;
          conf.current().perturbed_energy_derivatives.pert2d_crfs[n1] += pert2d_crfs;
...
//  if (t_interaction_spec::interaction_func == simulation::lj_crf_2d_func) {
//    std::cout << " perturbed_RF_excluded_interaction_innerloop 2d " << std::endl;
//  }
...
//    std::cout << "\te_rf " << e_rf << std::endl;
    ...
    {
      math::Vec f_rf;
      set_lambda_2d_other(topo.individual_lambda(simulation::lj_lambda)[n1][n1],
          topo.individual_lambda(simulation::lj_softness_lambda)[n1][n1],
          topo.individual_lambda(simulation::crf_lambda)[n1][n1],
          topo.individual_lambda(simulation::crf_softness_lambda)[n1][n1],
          topo.individual_lambda_derivative(simulation::lj_lambda)[n1][n1],
      ...
      double pert2d_crf, pert2d_crfs; 
      rf_soft_2d_interaction(r, q_i_a*q_i_a, q_i_b * q_i_b,
              mit->second.CRF_softcore(),
              f_rf, e_rf, de_rf, pert2d_crf, pert2d_crfs, true);

      conf.current().perturbed_energy_derivatives.pert2d_crf[n1] += 0.5 *pert2d_crf;
      conf.current().perturbed_energy_derivatives.pert2d_crfs[n1] += 0.5 *pert2d_crfs;
  ...
  for (; it != to; ++it) {
    ...
    switch (t_interaction_spec::interaction_func) {
      ...
      case simulation::lj_crf_2d_func:
      {
        ...
        double pert2d_crf, pert2d_crfs;
        rf_soft_2d_interaction(r, q_ij_a, q_ij_b, alpha_crf, f_rf, e_rf, de_rf, pert2d_crf, pert2d_crfs);
...
//  if (t_interaction_spec::interaction_func == simulation::lj_crf_2d_func) {
//    std::cout << " perturbed_self_energy_innerloop 2d " << std::endl;
//  }
