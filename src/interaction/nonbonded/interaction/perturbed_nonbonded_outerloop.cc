...
::perturbed_lj_crf_outerloop(topology::Topology & topo,
		...
		//changed    Pairlist const & pairlist,
                             Pairlist & pairlist,
			     Storage & storage)
{
...
{  
  ...
  for(i=0; i < size_i; ++i){
    ...
    for(j_it = pairlist.begin(i),  //changed
	  j_to = pairlist.end(i);  //changed
	j_it != j_to;
	++j_it){
...
::perturbed_electric_field_outerloop(topology::Topology & topo,
	...
	//changed  PairlistContainer const & pairlist,
        //changed  PairlistContainer const & perturbed_pairlist,
		   PairlistContainer & pairlist,
                   PairlistContainer & perturbed_pairlist,
                   Storage & storage,
                   Storage & storage_lr,
                   int rank)
{
...
{  
  ...
  if(!(sim.steps() % sim.param().pairlist.skip_step)){
    ...
    //changed  for(std::vector<unsigned int>::const_iterator
        for(Pairlist::iterator
              j_it = perturbed_pairlist.solute_long.begin(i),
              j_to = perturbed_pairlist.solute_long.end(i);
              j_it != j_to; ++j_it){
     ...
     //changed for(std::vector<unsigned int>::const_iterator
        for(Pairlist::iterator
              j_it = pairlist.solute_long.begin(i),
              j_to = pairlist.solute_long.end(i);
              j_it != j_to; ++j_it){
 ...
 //changed     for(std::vector<unsigned int>::const_iterator
        for(Pairlist::iterator
              j_it = pairlist.solvent_long.begin(i),
              j_to = pairlist.solvent_long.end(i);
              j_it != j_to; ++j_it){
  ...
  while (minfield >= minfield_param) {
   ...
   //changed   for(std::vector<unsigned int>::const_iterator
         for(Pairlist::iterator
              j_it = perturbed_pairlist.solute_short.begin(i),
	      j_to = perturbed_pairlist.solute_short.end(i);
              j_it != j_to; ++j_it){
   ...
   //changed   for(std::vector<unsigned int>::const_iterator
         for(Pairlist::iterator
              j_it = pairlist.solute_short.begin(i),
	      j_to = pairlist.solute_short.end(i);
              j_it != j_to; ++j_it){
 ...
 //changed     for(std::vector<unsigned int>::const_iterator
         for(Pairlist::iterator
              j_it = pairlist.solvent_short.begin(i),
	      j_to = pairlist.solvent_short.end(i);
              j_it != j_to; ++j_it){
