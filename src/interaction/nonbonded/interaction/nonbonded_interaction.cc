 ...
 * Constructor.
 */
interaction::Nonbonded_Interaction::Nonbonded_Interaction(Pairlist_Algorithm *pa, PairlistContainer *pc) //changed
: Interaction("NonBonded"),
m_pairlist_algorithm(pa),
m_pairlist_container(pc),  //changed
m_parameter(),
m_set_size(1),
m_exp_conf(NULL) {
...
interaction::Nonbonded_Interaction::~Nonbonded_Interaction() {
  ...
  delete m_pairlist_container; //changed
...
int interaction::Nonbonded_Interaction::init(topology::Topology & topo,
  ...
  if (sim.param().perturbation.perturbation) {
    for (int i = 0; i < m_set_size; ++i) {
      m_nonbonded_set.push_back(new Perturbed_Nonbonded_Set(*m_pairlist_algorithm, *m_pairlist_container,   //changed
            m_parameter, i, m_set_size));
    }
  } else if (sim.param().eds.eds) {
    DEBUG(16, "creating EDS nonbonded set");
    for (int i = 0; i < m_set_size; ++i) {
      m_nonbonded_set.push_back(new Eds_Nonbonded_Set(*m_pairlist_algorithm, *m_pairlist_container,    //changed
              m_parameter, i, m_set_size));
      DEBUG(16, "pushed back EDS nonbonded set");
    }
  } else {
    for (int i = 0; i < m_set_size; ++i) {
      m_nonbonded_set.push_back(new Nonbonded_Set(*m_pairlist_algorithm, *m_pairlist_container,    //changed
              m_parameter, i, m_set_size));
    }
  }
