/**
 * @file cuda_mpi_nonbonded_master.cc
 * methods of CUDA_MPI_Nonbonded_Master
 */
#ifdef XXMPI
#include <mpi.h>
#endif
 ...
 * Constructor.
 */
interaction::CUDA_MPI_Nonbonded_Master::CUDA_MPI_Nonbonded_Master(Pairlist_Algorithm *pa, PairlistContainer *pc)
 ...
 */
interaction::CUDA_MPI_Nonbonded_Master::~CUDA_MPI_Nonbonded_Master()
{
 ...
 */
int interaction::CUDA_MPI_Nonbonded_Master::
        ...
        simulation::Simulation & sim) 
{
  ...
  if ((sim.steps() % steps) == 0){
   ...
   // --------------------------------------------------
   // calculate pairlist on the GPU (master only)
  ...
  if(pairlist_update){
    DEBUG(6, "\tdoing gpu pairlist...");

  m_pairlist_algorithm->update(topo, conf, sim,
                          *m_pairlist_container,
                          rank, topo.num_atoms(), num_threads);
  }
 ...
 */
int interaction::CUDA_MPI_Nonbonded_Master::init(topology::Topology & topo,
        configuration::Configuration & conf,
        simulation::Simulation & sim,
        std::ostream & os,
        bool quiet) 
