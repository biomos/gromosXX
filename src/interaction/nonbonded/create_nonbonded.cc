...
#ifdef XXCUDA
#include "../../interaction/nonbonded/interaction/cuda_mpi_nonbonded_master.h"
 ...
 )
{
  ...
  if(!quiet){
    ...
    if (sim.param().pairlist.grid == 0)
     ...
     else if (sim.param().pairlist.grid == 3){
      os << "\t" << setw(20) << left << "Pairlist Algorithm" << setw(30)
	   << left << "CUDA pairlist" << right << "\n";
    }
...
#endif
 //CHANGE TO DYNAMIC !!!!!!!!!!
 PairlistContainer * pc;
 // calculate pairlist size
 sim.param().pairlist.pairlist_height = 1120;

 pc = new PairlistContainer(topo.num_atoms(), sim.param().pairlist.pairlist_height);

#if defined(OMP)
  Nonbonded_Interaction * ni = new OMP_Nonbonded_Interaction(pa, pc);
#elif defined(XXMPINC)
  Nonbonded_Interaction * ni;

  if (sim.mpi){
    int rank = MPI::COMM_WORLD.Get_rank();
    if (rank == 0)
      ni = new MPI_Nonbonded_Master(pa, pc);
    else
      ni = new MPI_Nonbonded_Slave(pa, pc);
  }
  else
    ni = new Nonbonded_Interaction(pa, pc);
  ...
  if (sim.mpi){
    ...
    if (rank == 0) {
         if (sim.param().pairlist.grid == 3){
             ni = new CUDA_MPI_Nonbonded_Master(pa, pc);
         } else {
             ni = new MPI_Nonbonded_Master(pa, pc);
         }
     } else {
         if (sim.param().pairlist.grid == 3){
             ni = new CUDA_MPI_Nonbonded_Slave(pa, pc);
         } else {
             ni = new MPI_Nonbonded_Slave(pa, pc);
         }
     }
...
#else
  Nonbonded_Interaction * ni = new Nonbonded_Interaction(pa, pc);
#endif
