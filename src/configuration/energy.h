...
namespace configuration
{
  ...
  public:
    ...
    /** 
    * total A_lj for each lambda (extended TI)
    */
    std::vector<double> A_lj_total;

    /** 
    * total B_lj for each lambda (extended TI)
    */
    std::vector<double> B_lj_total;

    /**
    * total A_crf for each lambda (extended TI)
    */
    std::vector<double> A_crf_total;

    /**
    * total B_crf for each lambda (extended TI)
    */
    std::vector<double> B_crf_total;

    /**
    * kinetic energy for each lambda (extended TI)
    */
    std::vector<double> AB_kinetic;

    /**
    * bond stretching energy for each lambda (extended TI)
    */
    std::vector<double> AB_bond;

    /** 
    * bond angle energy for each lambda (extended TI)
    */
    std::vector<double> AB_angle;

    /**
    * improper dihedral energy for each lambda (extended TI)
    */
    std::vector<double> AB_improper;

    /**
    * dihedral energy for each lambda (extended TI)
    */
    double A_dihedral;
    double B_dihedral;

    /**
    * free energy for constant lambda, excluding dlamI_dLAM (2D perturbation)
    */
    std::vector<double> pert2d_lj;
    std::vector<double> pert2d_ljs;
    std::vector<double> pert2d_crf;
    std::vector<double> pert2d_crfs;
    std::vector<double> pert2d_kin;
    ...
    /**
     * A_lj energies for [lam][groupi][groupj] (extended TI)   
     */
    std::vector<std::vector<std::vector<double> > > A_lj_energy;

    /**
     * B_lj energies for [lam][groupi][groupj] (extended TI)
     */
    std::vector<std::vector<std::vector<double> > > B_lj_energy;

    /**
     * A_crf energies for [lam][groupi][groupj] (extended TI)   
     */
    std::vector<std::vector<std::vector<double> > > A_crf_energy;

    /**
     * B_crf energies for [lam][groupi][groupj] (extended TI)
     */
