...
void configuration::Energy::zero(bool potential, bool kinetic)
{
  ...
  if (potential){
    ...
    pert2d_lj.assign(pert2d_lj.size(),0.0);
    pert2d_ljs.assign(pert2d_ljs.size(),0.0);
    pert2d_crf.assign(pert2d_crf.size(),0.0);
    pert2d_crfs.assign(pert2d_crfs.size(),0.0);
    pert2d_kin.assign(pert2d_kin.size(),0.0);
    ...
    // for extended TI: 
    if (A_lj_total.size()){
      DEBUG(10, "setting A_lj_energy to 0");
      DEBUG(10, "  nr_lambdas in A_lj_energy: " << unsigned(A_lj_total.size()));
     ...
     unsigned int nr_lambdas)
{
  ...
  if (energy_groups){
    ...
    DEBUG(8, "resizing A_lj_energy to " << nr_lambdas);
  ...
  //for 2D perturbation
  pert2d_lj.resize(energy_groups);
  pert2d_ljs.resize(energy_groups);
  pert2d_crf.resize(energy_groups);
  pert2d_crfs.resize(energy_groups);
  pert2d_kin.resize(energy_groups);
